...
本sample以Llama2-7b，qwen2-7b，qwen3-8b模型，pileval数据，wikitext2数据集为示例，请用户自行下载，并根据实际保存目录修改utils.py文件中get_llama2，get_loaders， get_calib_dataset等获取模型或数据函数中的读取路径。
...
minmax算法支持仅权重量化和全量化，支持的量化类型以及量化配置：
...
```python
python3 src/run_qwen3_samples.py
```
...
| :-: | :-: | :-: | :-: | :-: |
|LLAMA2-7B|pileval|wikitext2|5.472|5.477|
|QWEN2-7B|pileval|wikitext2|7.137|7.139|
|QWEN3-8B|pileval|wikitext2|9.715|9.692|
