...
def get_qwen3(model, seqlen=2048):
    '''If model is specified from ['8b', '30b'], then we load official pretrained model;
       If you want to load checkpoints other than the official ones, please specifiy the model path,
       otherwise please choose from ['8b', '30b'] for better clarity
    '''
    if model in ['8b', '30b']:
        model_path = f'/data/Models/pytorch/Qwen3/Qwen3_{model}'
        print(f'Getting official pretrained Qwen3_{model}')
    else:
        model_path = model
