...
|inputs.type|str|量化后权重类型|'int8'/'float8_e4m3fn'|全量化场景不支持配置权重量化类型int4|
|inputs.symmetric|bool|对称量化|TRUE/FALSE|量化数据类型为float8_e4m3fn时，只支持对称量化|
...
| 模型 | 校准集 | 数据集 | 量化前PPL | int8量化后PPL  | float8_e4m3fn*float4_e2m1量化后PPL | 
| :-: | :-: | :-: | :-: | :-: | :-: |
