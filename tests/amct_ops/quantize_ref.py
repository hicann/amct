...
def construct_fp4_from_components(
    new_biased_exp,
    ...
    mx4_value = torch.bitwise_or(
        torch.bitwise_left_shift(new_biased_exp, m_bits - 1), mantissa
    )
    mx4_value = torch.bitwise_or(
        torch.bitwise_left_shift(sign_bit, e_bits + m_bits - 1), mx4_value
    )
...
def mantissa_calc(
    ...
    mantissa = torch.bitwise_right_shift(
        mantissa, fp32_sig_bits + exp_diff - m_bits - 1
    ).to(torch.int8)
    ...
    return mantissa, new_biased_exp


def shared_exponents_for_each_group(
    a,
    rounding_mode,
    mbits,
    e_bits,
...
def py_quantize_mx4(
    ...
    Args:
        ...
        casting (bool): Whether to use stochastic rounding when downcasting.
    ...
    e_bits = ebits
    m_bits = mbits + 1
    fp4_exp_bias = get_mx4_exp_bias(e_bits)
    rand_mask = (1 << (FP32_EXP_OFFSET - mbits)) - 1
    mantissa_overflow_threshold = (1 << m_bits) - 1
    exponent_overflow_threshold = (1 << e_bits) - 1
    implicit_1_mask = (1 << (m_bits - 1)) - 1
    ...
    a = a.contiguous().view(a.numel() // group_size, group_size)

    shared_exp, scale = shared_exponents_for_each_group(a, rounding_mode, mbits, e_bits)
    ...
    mantissa, new_biased_exp = mantissa_calc(
        biased_exp,
        trailing_mantissa,
        exp_diff,
        m_bits,
    ...
    return construct_fp4_from_components(
        new_biased_exp,
        exponent_overflow_threshold,
        sign_bit,
        e_bits,
