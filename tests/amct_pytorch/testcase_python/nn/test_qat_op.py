...
quant_configs = [
    {'retrain_weight_config': {}, RETRAIN_DATA_CONFIG: {}},
    {
        'retrain_weight_config': {'weights_retrain_algo': 'ulq_retrain'},
        RETRAIN_DATA_CONFIG: {'batch_num': 3},
    },
    {
        'retrain_weight_config': {'channel_wise': False},
        RETRAIN_DATA_CONFIG: {'batch_num': 3, 'clip_min': -1.0, 'clip_max': 1.0},
    },
    {
        ...
        RETRAIN_DATA_CONFIG: {'batch_num': 3, 'fixed_min': True},
    },
    {
        'retrain_weight_config': {
            'weights_retrain_algo': 'ulq_retrain',
...
class TestQatOp(unittest.TestCase):
    ...
    def setUp(self):
        if os.path.exists(
            './amct_log/amct_pytorch.classic.graph_based.amct_pytorch.log'
        ):
            os.remove('./amct_log/amct_pytorch.classic.graph_based.amct_pytorch.log')
            LOGGER.logi(
                'amct_pytorch.classic.graph_based.amct_pytorch.log is initialized successfully.'
            )
    ...
    def test_qat_base_init_success(self):
        quant_conf = {RETRAIN_DATA_CONFIG: {'clip_max': 1.0, 'clip_min': -1.0}}
    ...
    def test_qat_base_init_failed_wrong_config_data_type(self):
        ...
        with self.assertRaises(ValueError):
            ...
            with open(
                './amct_log/amct_pytorch.classic.graph_based.amct_pytorch.log', 'r'
            ) as f:
                log_content = f.read()
    ...
    def test_qat_base_init_failed_wrong_config_data_scope(self):
        ...
        wrong_quant_conf = {RETRAIN_DATA_CONFIG: {'batch_num': -1}}
        with self.assertRaises(ValueError):
            Conv2dQAT(1, 1, 1, config=wrong_quant_conf)

        wrong_quant_conf = {RETRAIN_DATA_CONFIG: {'clip_min': 1.0, 'clip_max': 1.0}}
        with self.assertRaises(ValueError):
            Conv2dQAT(1, 1, 1, config=wrong_quant_conf)
...
class TestLinearQAT(unittest.TestCase):
    ...
    def test_lineard_qat_forward(self):
        qat_op = LinearQAT(
            16, 10, config={'retrain_weight_config': {'channel_wise': False}}
        )
    ...
    def test_lineard_qat_from_float_01(self):
        ...
        qat_linear = LinearQAT.from_float(
            ori_op, config={'retrain_weight_config': {'channel_wise': False}}
        )
...
class NetConv1d(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv1d(2, 16, kernel_size=3, bias=False), nn.BatchNorm1d(16)
        )
        ...
        self.layer3 = nn.Sequential(
            nn.Conv1d(16, 16, kernel_size=3, groups=16), nn.BatchNorm1d(16)
        )
        ...
        self.layer5 = nn.Sequential(
            nn.Conv1d(16, 32, kernel_size=3, groups=4), nn.BatchNorm1d(32)
        )
...
class NetConv1dQAT(nn.Module):
    def __init__(self):
        ...
        # conv + bn
        self.layer1 = nn.Sequential(
            Conv1dQAT(2, 16, kernel_size=3, bias=False), nn.BatchNorm1d(16)
        )
        self.layer2 = nn.Sequential(
        ...
        # depthwise_conv + bn
        self.layer3 = nn.Sequential(
            Conv1dQAT(16, 16, kernel_size=3, groups=16), nn.BatchNorm1d(16)
        )
        self.layer4 = nn.Sequential(
        ...
        # group_conv + bn
        self.layer5 = nn.Sequential(
            Conv1dQAT(16, 32, kernel_size=3, groups=4), nn.BatchNorm1d(32)
        )
        self.layer6 = nn.Sequential(
    ...
    def test_conv1_qat_convert_from_ori_op(self):
        ...
        for name, module in net_conv1d.named_modules():
            if isinstance(module, nn.Conv1d):
                qat_module = Conv1dQAT.from_float(module)
...
class NetConvTranspose1dQAT(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            ConvTranspose1dQAT(
                2, 16, kernel_size=3, bias=False, config=quant_configs[0]
            ),
            nn.BatchNorm1d(16),
        )
        self.layer2 = nn.Sequential(
            ConvTranspose1dQAT(
                16, 16, kernel_size=3, bias=True, config=quant_configs[1]
            ),
            nn.BatchNorm1d(16),
            nn.ReLU(inplace=True),
        )
        # depthwise_conv + bn
        self.layer3 = nn.Sequential(
            ConvTranspose1dQAT(16, 16, kernel_size=3, config=quant_configs[2]),
            nn.BatchNorm1d(16),
        )
        self.layer4 = nn.Sequential(
            ConvTranspose1dQAT(16, 16, kernel_size=3, config=quant_configs[3]),
            nn.BatchNorm1d(16),
            nn.ReLU(inplace=True),
        )
        # group_conv + bn
        self.layer5 = nn.Sequential(
            ConvTranspose1dQAT(16, 32, kernel_size=3, config=quant_configs[4]),
            nn.BatchNorm1d(32),
        )
        self.layer6 = nn.Sequential(
            ConvTranspose1dQAT(32, 8, kernel_size=3, config=quant_configs[5]),
            nn.BatchNorm1d(8),
            nn.ReLU(inplace=True),
        )
...
class NetConvTranspose1d(nn.Module):
    def __init__(self):
        ...
        # conv + bn
        self.layer1 = nn.Sequential(
            nn.ConvTranspose1d(2, 16, kernel_size=3, bias=False), nn.BatchNorm1d(16)
        )
        self.layer2 = nn.Sequential(
        ...
        # depthwise_conv + bn
        self.layer3 = nn.Sequential(
            nn.ConvTranspose1d(16, 16, kernel_size=3, groups=16), nn.BatchNorm1d(16)
        )
        self.layer4 = nn.Sequential(
        ...
        # group_conv + bn
        self.layer5 = nn.Sequential(
            nn.ConvTranspose1d(16, 32, kernel_size=3), nn.BatchNorm1d(32)
        )
        self.layer6 = nn.Sequential(
...
class TestMatMulQAT(unittest.TestCase):
    ...
    def test_acts_quant_init_success(self):
        ...
        result = matmul_op.acts_quant_init(
            torch.randn(30, 30),
            matmul_op.acts_quant_params.get('input'),
            matmul_op.input_init_module,
        )
    ...
    def test_matmul_op_infer_success(self):
        ...
        quantized_out = ort_session.run(
            None,
            {
                ort_session.get_inputs()[0].name: input_data.cpu().numpy(),
