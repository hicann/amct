...
class TestConfigParse(unittest.TestCase):
    ...
    def test_int4_awq_cfg(self):
        detail_config = parse_config(
            self.test_model, INT4_AWQ_WEIGHT_QUANT_CFG, AlgorithmRegistry
        )

        self.assertEqual(len(detail_config.keys()), 2)
        self.assertEqual(detail_config.get(LAYER_LINEAR1).get(KEY_BATCH_NUM), 1)
        self.assertEqual(
            detail_config.get(LAYER_LINEAR1).get(WEIGHTS_CFG).get('quant_type'), 'int4'
        )
        self.assertTrue(
            detail_config.get(LAYER_LINEAR1).get(WEIGHTS_CFG).get(KEY_SYMMETRIC)
        )
        self.assertEqual(
            detail_config.get(LAYER_LINEAR1).get(WEIGHTS_CFG).get(KEY_STRATEGY),
            'channel',
        )
        self.assertFalse(
            detail_config.get(LAYER_LINEAR1).get(INPUTS_CFG).get('enable_quant')
        )
        self.assertEqual(len(detail_config.get(LAYER_LINEAR1).get(ALGORITHM_KEY)), 1)
        self.assertEqual(
            detail_config.get(LAYER_LINEAR1)
            .get(ALGORITHM_KEY)
            .get('awq')
            .get('grids_num'),
            20,
    ...
    def test_int4_gptq_cfg(self):
        detail_config = parse_config(
            self.test_model, INT4_GPTQ_WEIGHT_QUANT_CFG, AlgorithmRegistry
        )
        ...
        self.assertEqual(len(detail_config.get(LAYER_LINEAR1).get(ALGORITHM_KEY)), 1)
        self.assertEqual(
            list(detail_config.get(LAYER_LINEAR1).get(ALGORITHM_KEY))[0], 'gptq'
        )

    def test_int8_minmax_cfg(self):
        model_bfloat16 = copy.deepcopy(self.test_model).to(torch.bfloat16)
        detail_config = parse_config(
            model_bfloat16, INT8_MINMAX_WEIGHT_QUANT_CFG, AlgorithmRegistry
        )

        self.assertEqual(len(detail_config.keys()), 3)
        self.assertEqual(detail_config.get(LAYER_LINEAR1).get(KEY_BATCH_NUM), 1)
        self.assertEqual(
            detail_config.get(LAYER_LINEAR1).get(WEIGHTS_CFG).get('quant_type'), 'int8'
        )
        self.assertTrue(
            detail_config.get(LAYER_LINEAR1).get(WEIGHTS_CFG).get(KEY_SYMMETRIC)
        )
        ...
        self.assertEqual(len(detail_config.get(LAYER_LINEAR1).get(ALGORITHM_KEY)), 1)
        self.assertEqual(
            list(detail_config.get(LAYER_LINEAR1).get(ALGORITHM_KEY))[0], 'minmax'
        )

    def test_int8_smooth_cfg(self):
        model_bfloat16 = copy.deepcopy(self.test_model).to(torch.bfloat16)
        detail_config = parse_config(
            model_bfloat16, INT8_SMOOTHQUANT_CFG, AlgorithmRegistry
        )
        ...
        self.assertEqual(
            detail_config.get(LAYER_LINEAR1).get(INPUTS_CFG).get('quant_type'), 'int8'
        )
        self.assertTrue(
            detail_config.get(LAYER_LINEAR1).get(INPUTS_CFG).get(KEY_SYMMETRIC)
        )
        self.assertEqual(
            detail_config.get(LAYER_LINEAR1).get(INPUTS_CFG).get(KEY_STRATEGY),
            STRATEGY_TENSOR,
        )
        self.assertEqual(len(detail_config.get(LAYER_LINEAR1).get(ALGORITHM_KEY)), 1)
        self.assertEqual(
    ...
    def test_int8_int4_minmax_cfg(self):
        ...
        self.assertEqual(len(detail_config.keys()), 2)
        self.assertEqual(
            detail_config.get('linear1').get('weights_cfg').get('quant_type'), 'int4'
        )
        self.assertEqual(
            detail_config.get('linear1').get('weights_cfg').get('symmetric'), True
        )
        self.assertEqual(
            detail_config.get('linear1').get('weights_cfg').get('strategy'), 'channel'
        )
        self.assertEqual(
            detail_config.get('linear1').get('inputs_cfg').get('quant_type'), 'int8'
        )
        self.assertEqual(
            detail_config.get('linear1').get('inputs_cfg').get('symmetric'), True
        )
        self.assertEqual(
            detail_config.get('linear1').get('inputs_cfg').get('strategy'), 'tensor'
        )
        self.assertEqual(
            list(detail_config.get('linear1').get('algorithm'))[0], 'minmax'
        )
    ...
    def test_invalid_wts_symmetric(self):
        ...
        try:
            ...
            self.assertIn(
                'Weights symmetric only support bool [True, False], but got', str(e)
            )
    ...
    def test_smooth_invalid_grids_num(self):
        ...
        try:
            ...
            self.assertIn(
                'Smoothquant smooth_strength only support float (0, 1), but got', str(e)
            )
    ...
    def test_unsupported_quant_type_comb(self):
        ...
        try:
            parse_config(
                self.test_model, unsupported_quant_type_comb_cfg, AlgorithmRegistry
            )
        except Exception as e:
            self.assertIn(
                'Do not support combination int8 int4 of act and weight quant dtype',
                str(e),
            )
    ...
    def test_unsupported_algo_for_quant_type_comb(self):
        ...
        try:
            parse_config(
                self.test_model, unsupported_algo_quant_type_comb_cfg, AlgorithmRegistry
            )
        except Exception as e:
            self.assertIn(
                'Algorithm gptq do not support act and weight quant dtype int8 int8',
                str(e),
            )
    ...
    def test_skip_layers(self):
        ...
        detail_config = parse_config(
            self.test_model, int4_awq_skip_layer_cfg, AlgorithmRegistry
        )
    ...
    def test_group_size_none_strategy_group(self):
        ...
        try:
            ...
            self.assertIn(
                'Weights group_size is necessary, when weights strategy is group',
                str(e),
            )
    ...
    def test_group_size_32_strategy_other(self):
        ...
        try:
            ...
            self.assertIn(
                'Weights group_size only support strategy group, but got ', str(e)
            )
    ...
    def test_int8_int8_group_invalid_cfg(self):
        ...
        try:
            ...
            self.assertIn(
                'int8 int8 only support symmetric weight quantization', str(e)
            )
    ...
    def test_customize_algo_cfg(self):
        ...
        AlgorithmRegistry.register(
            ALGO_NAME_A, 'Linear', CustomQuant, CustomDeployQuant
        )
    ...
    def test_int8_int8_token_asymetric_invalid_cfg(self):
        ...
        try:
            ...
            self.assertIn(
                'Inputs strategy token do not support asymmetric quantization', str(e)
            )
    ...
    def test_int8_int8_weight_asymmetric_invalid_cfg(self):
        cfg = {
            'batch_num': 1,
            'quant_cfg': {
                'weights': {'type': 'int8', 'symmetric': False, 'strategy': 'channel'},
                'inputs': {
                    'type': 'int8',
                    'symmetric': True,
                    'strategy': 'tensor',
    ...
    def test_int8_int4_weight_asymmetric_invalid_cfg(self):
        cfg = {
            'batch_num': 1,
            'quant_cfg': {
                'weights': {'type': 'int4', 'symmetric': False, 'strategy': 'channel'},
                'inputs': {
                    'type': 'int8',
                    'symmetric': True,
                    'strategy': 'tensor',
        ...
        try:
            ...
            self.assertIn(
                'int8 int4 only support symmetric weight quantization', str(e)
            )
    ...
    def test_int8_int4_group_invalid_cfg(self):
        ...
        try:
            ...
            self.assertIn(
                'int8 int4 only support weight quant strategy tensor or channel', str(e)
            )
    ...
    def test_int8_int4_token_invalid_cfg(self):
        ...
        try:
            ...
            self.assertIn(
                'int8 int4 only support activation quant strategy tensor', str(e)
            )
    ...
    def test_int8_int4_activation_asymmetric_cfg(self):
        cfg = {
            'batch_num': 1,
            'quant_cfg': {
                'weights': {'type': 'int4', 'symmetric': True, 'strategy': 'tensor'},
                'inputs': {
                    'type': 'int8',
                    'symmetric': False,
                    'strategy': 'tensor',
        ...
        detail_config = parse_config(
            copy.deepcopy(self.test_model).to(torch.float16), cfg, AlgorithmRegistry
        )
        self.assertEqual(len(detail_config.keys()), 2)
        self.assertEqual(
            detail_config.get('linear1').get('inputs_cfg').get('symmetric'), False
        )
    ...
    def test_int8_int4_dtype_skip_cfg(self):
        ...
        detail_config = parse_config(
            copy.deepcopy(self.test_model).to(torch.bfloat16), cfg, AlgorithmRegistry
        )
        self.assertEqual(len(detail_config.keys()), 0)
    ...
    def test_float8_float4_smooth_cfg(self):
        ...
        self.assertEqual(
            detail_config.get(LINEAR2).get(WEIGHTS_CFG).get('quant_type'), FLOAT4_E2M1
        )
        self.assertTrue(detail_config.get(LINEAR2).get(WEIGHTS_CFG).get(KEY_SYMMETRIC))
        self.assertEqual(
            detail_config.get(LINEAR2).get(WEIGHTS_CFG).get(KEY_STRATEGY), 'group'
        )
        self.assertEqual(
            detail_config.get(LINEAR2).get(INPUTS_CFG).get('quant_type'), FLOAT8_E4M3FN
        )
        self.assertTrue(detail_config.get(LINEAR2).get(INPUTS_CFG).get(KEY_SYMMETRIC))
        self.assertEqual(
            detail_config.get(LINEAR2).get(INPUTS_CFG).get(KEY_STRATEGY),
            STRATEGY_TENSOR,
        )
        self.assertEqual(len(detail_config.get(LINEAR2).get(ALGORITHM_KEY)), 1)
        self.assertEqual(
            detail_config.get(LINEAR2)
            .get(ALGORITHM_KEY)
            .get(SMOOTH_QUANT)
            .get(SMOOTH_STRENGTH),
            0.5,
    ...
    def test_float8_float4_minmax_cfg(self):
        ...
        self.assertEqual(len(detail_config.get(LINEAR2).get(ALGORITHM_KEY)), 1)
        self.assertEqual(
            list(detail_config.get(LINEAR2).get(ALGORITHM_KEY))[0], 'minmax'
        )
    ...
    def test_float8_float4_symmetric_invalid_cfg(self):
        ...
        try:
            detail_config = parse_config(
                model_bfloat16, cfg_weights_asymmetric, AlgorithmRegistry
            )
            self.assertEqual(len(detail_config.keys()), 0)
        except Exception as e:
            self.assertIn(
                'Weights symmetric only support to be True when weight quant_type is float4_e2m1',
                str(e),
            )

        try:
            detail_config = parse_config(
                model_bfloat16, cfg_inputs_asymmetric, AlgorithmRegistry
            )
            self.assertEqual(len(detail_config.keys()), 0)
        except Exception as e:
            self.assertIn(
                'Inputs symmetric is unsupported to be False when Inputs quant_type is float8_e4m3fn',
                str(e),
            )
    ...
    def test_float8_float4_strategy_invalid_cfg(self):
        ...
        try:
            detail_config = parse_config(
                model_bfloat16, cfg_weights_tensor, AlgorithmRegistry
            )
            self.assertEqual(len(detail_config.keys()), 0)
        except Exception as e:
            self.assertIn(
                'act_dtype and wts_dtype float8_e4m3fn float4_e2m1 do not support '
                'weight quant strategy tensor',
                str(e),
            )

        try:
            detail_config = parse_config(
                model_bfloat16, cfg_weights_channel, AlgorithmRegistry
            )
            self.assertEqual(len(detail_config.keys()), 0)
        except Exception as e:
            self.assertIn(
                'act_dtype and wts_dtype float8_e4m3fn float4_e2m1 do not support '
                'weight quant strategy channel',
                str(e),
            )

        try:
            detail_config = parse_config(
                model_bfloat16, cfg_inputs_token, AlgorithmRegistry
            )
            self.assertEqual(len(detail_config.keys()), 0)
        except Exception as e:
            self.assertIn(
                'act_dtype and wts_dtype float8_e4m3fn float4_e2m1 do not support '
                'activation quant strategy token',
                str(e),
            )
    ...
    def test_float8_float4_groupsize_invalid_cfg(self):
        ...
        try:
            detail_config = parse_config(
                model_bfloat16, cfg_weights_tensor_groupsize, AlgorithmRegistry
            )
            self.assertEqual(len(detail_config.keys()), 0)
        except Exception as e:
            self.assertIn(
                'Weights group_size only support strategy group, but got tensor', str(e)
            )

        try:
            detail_config = parse_config(
                model_bfloat16, cfg_weights_without_groupsize, AlgorithmRegistry
            )
            self.assertEqual(len(detail_config.keys()), 0)
        except Exception as e:
            self.assertIn(
    ...
    def test_float8_float4_algorithm_invalid_cfg(self):
        ...
        for algorithm in algorithm_not_support:
            ...
            try:
                detail_config = parse_config(
                    model_bfloat16, cfg_algorithm, AlgorithmRegistry
                )
                self.assertEqual(len(detail_config.keys()), 0)
            except Exception as e:
                self.assertIn(
                    f'Algorithm {algorithm} do not support act and weight quant dtype '
            ...
            try:
                detail_config = parse_config(
                    model_bfloat16, cfg_algorithm_awq, AlgorithmRegistry
                )
                self.assertEqual(len(detail_config.keys()), 0)
            except Exception as e:
                self.assertIn(
                    'Algorithm awq do not support act and weight quant dtype '
                    'float8_e4m3fn float4_e2m1',
                    str(e),
                )
    ...
    def test_hifloat8_hifloat8_cfg(self):
        ...
        for weights_strategy in weights_strategy_support:
            ...
            self.assertEqual(detail_config.get(LAYER_LINEAR1).get(KEY_BATCH_NUM), 1)
            self.assertEqual(
                detail_config.get(LAYER_LINEAR1).get(WEIGHTS_CFG).get('quant_type'),
                HIFLOAT8,
            )
            self.assertTrue(
                detail_config.get(LAYER_LINEAR1).get(WEIGHTS_CFG).get(KEY_SYMMETRIC)
            ...
            self.assertEqual(
                detail_config.get(LAYER_LINEAR1).get(INPUTS_CFG).get('quant_type'),
                HIFLOAT8,
            )
            self.assertTrue(
                detail_config.get(LAYER_LINEAR1).get(INPUTS_CFG).get(KEY_SYMMETRIC)
            ...
            self.assertEqual(
                len(detail_config.get(LAYER_LINEAR1).get(ALGORITHM_KEY)), 1
            )
            self.assertEqual(
                list(detail_config.get(LAYER_LINEAR1).get(ALGORITHM_KEY))[0], 'ofmr'
            )
    ...
    def test_hifloat8_hifloat8_conv2d_cfg(self):
        ...
        for weights_strategy in weights_strategy_support:
            ...
            self.assertEqual(
                detail_config.get(CONV2D1).get(WEIGHTS_CFG).get('quant_type'), HIFLOAT8
            )
            self.assertTrue(
                detail_config.get(CONV2D1).get(WEIGHTS_CFG).get(KEY_SYMMETRIC)
            )
            self.assertEqual(
                detail_config.get(CONV2D1).get(WEIGHTS_CFG).get(KEY_STRATEGY),
                f"{weights_strategy}",
            )
            self.assertEqual(
                detail_config.get(CONV2D1).get(INPUTS_CFG).get('quant_type'), HIFLOAT8
            )
            self.assertTrue(
                detail_config.get(CONV2D1).get(INPUTS_CFG).get(KEY_SYMMETRIC)
            )
            self.assertEqual(
                detail_config.get(CONV2D1).get(INPUTS_CFG).get(KEY_STRATEGY),
                STRATEGY_TENSOR,
            )
            self.assertEqual(len(detail_config.get(CONV2D1).get(ALGORITHM_KEY)), 1)
            self.assertEqual(
                list(detail_config.get(CONV2D1).get(ALGORITHM_KEY))[0], 'ofmr'
            )
    ...
    def test_multi_algo_fail_cfg(self):
        cfg = {
            ...
            ALGORITHM_KEY: {'minmax', 'gptq'},
        }
        model_bfloat16 = copy.deepcopy(TestModel()).to(torch.bfloat16)
        try:
            parse_config(model_bfloat16, cfg, AlgorithmRegistry)
        except Exception as e:
            self.assertIn('One src_op only support one algorithm, current algo', str(e))
    ...
    def test_int8_minmax_skip_lm_head_with_hidden_layers(self):
        ...
        class RealisticLLM(nn.Module):
            ...
            def forward(self, x):
                return self.lm_head(
                    self.mlp_down(torch.relu(self.mlp_gate(x) * self.mlp_up(x)))
                )

        model = RealisticLLM().to(torch.bfloat16)
        detail_config = parse_config(
            model, INT8_MINMAX_WEIGHT_QUANT_CFG, AlgorithmRegistry
        )
    ...
    def test_int8_minmax_lm_head_auto_skipped_by_dim_limit(self):
        ...
        self.assertFalse(
            check_quant_op_constraint(
                lm_head, 'lm_head', 'NOT_QUANTIZE int8', quant_config
            ),
        ...
        self.assertTrue(
            check_quant_op_constraint(
                llama_lm_head, 'lm_head', 'NOT_QUANTIZE int8', quant_config
            ),
...
class TestNpuWeightQuantDimLimit(unittest.TestCase):
    ...
    def test_boundary_and_exceeds(self):
        ...
        self.assertTrue(
            self.check_fn(
                nn.Linear(4096, 65535, bias=False),
                'layer_n_boundary',
                'NOT_QUANTIZE int8',
                self.quant_config,
            )
        ...
        self.assertTrue(
            self.check_fn(
                nn.Linear(65535, 4096, bias=False),
                'layer_k_boundary',
                'NOT_QUANTIZE int8',
                self.quant_config,
            )
        ...
        self.assertFalse(
            self.check_fn(
                nn.Linear(4096, 65536, bias=False),
                'layer_n_exceed',
                'NOT_QUANTIZE int8',
                self.quant_config,
            )
        ...
        self.assertFalse(
            self.check_fn(
                nn.Linear(65536, 4096, bias=False),
                'layer_k_exceed',
                'NOT_QUANTIZE int8',
                self.quant_config,
            )
    ...
    def test_real_model_vocab_sizes(self):
        ...
        self.assertFalse(
            self.check_fn(
                nn.Linear(4096, 151936, bias=False),
                'qwen2_lm_head',
                'NOT_QUANTIZE int8',
                self.quant_config,
            )
    ...
    def test_full_quant_combos_not_affected_by_dim_limit(self):
        ...
        for comb in full_quant_combs:
            ...
            result = self.check_fn(
                large_layer, 'oversized_full_quant', comb, self.quant_config
            )
    ...
    def test_dim_limit_does_not_mask_other_checks(self):
        ...
        self.assertFalse(
            self.check_fn(
                bad_cin_layer, 'bad_cin', 'float8_e4m3fn float4_e2m1', self.quant_config
            ),
        ...
        self.assertTrue(
            self.check_fn(
                good_layer, 'good_layer', 'float8_e4m3fn float4_e2m1', self.quant_config
            ),
