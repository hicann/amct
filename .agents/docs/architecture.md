...
`.agents/` 是**内容唯一源**（git tracked）；`.claude/`、`.opencode/` 下按类型分两种方式投影：

- **git-tracked 符号链接**（源改即时生效）：`skills` / `agents` / `hooks` / `docs` 目录
- **git-tracked 真实文件**（各端独立持有，不在 `.agents/` 下）：`settings.json`（`.claude/`）、`opencode.json`（`.opencode/`）

视图分布：`.claude/{skills,agents,hooks,docs,settings.json}`；`.opencode/{skills,agents,docs,opencode.json}`。配置文件为真实文件（兼容 Windows），内容目录为 symlink（clone 自动还原）。系统提示统一由根目录 `AGENTS.md` 提供。一处源、双端一致，不双份维护。
...
- 启动：clone 仓库后即可直接在 Claude Code 或 OpenCode 中启动（symlink 视图随 clone 自动生效）。
