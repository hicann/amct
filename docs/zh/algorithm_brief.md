...
| 算法 | INT8 | INT4 | MXFP8 | MXFP4 | HiFloat8 | HiFloat4 |
|--------|:----------:|:----------:|:-----------:|:-----------:|:--------:|:--------:|
| Min-Max    | ✅ | ✅ | — | — | — | — |
| AWQ        | ✅ | ✅ | — | — | — | — |
| GPTQ       | ✅ | ✅ | — | ✅ | ✅ | ✅ |
...
ARQ （Adaptive Range Quantization）算法是对权重直接量化的算法。该算法提供了两种方式，channel\_wise（是否对每个channel采用不同的量化因子，参数解释请参见[训练后量化配置参数](./context/ptq_config_param.md)量化和非channel\_wise量化，量化原理图如下所示：

**图 1**  非channel\_wise量化<a name="fig7600113961719"></a>  
![](./figures/nonchannel_wise_quant.png "非channel_wise量化")

**图 2**  channel\_wise量化<a name="fig6351104815177"></a>  
![](./figures/channel_wise_quant.png "channel_wise量化")

量化配置中ARQuantize（以PyTorch框架为例，参数解释请参见[表1](./context/ptq_config.md#zh-cn_topic_0240188735_table1225503375617)）提供的参数channel\_wise用来选择量化方式：
...
AutoRound可学习Rounding Offset，是面向LLM的轻量级训练后量化算法，核心是把“舍入编译V”做成可学习参数，并可联合调节min/max scale，让权重在低比特量化时选择更优的舍入方向。
算法详细说明请参见：[Optimize Weight Rounding via Signed Gradient Descent for the Quantization of LLMs](https://arxiv.org/abs/2309.05516)。

说明：`autoround`带自定义 `quantize()` hook，目前PTQ fake quant可用，但部署导出中 `export_deploy()` 暂不支持该类hook。
...
说明：针对 HiFloat4 格式（对应算法为 HiGPTQ），GPTQ 会自动改为逐 64 元素分组量化，以匹配 HiFloat4 的三级层次化 block scale，分组内的列需联合求解，无法像 int/mxfp 那样按单列独立量化；其余 Hessian 校准流程与标准 GPTQ 一致，且仅支持权重量化。

### LWC

LWC（Learnable Weight Clipping，可学习权重裁剪范围）是面向神经网络量化，尤其是LLM训练后量化的可微权重范围优化技术，在可学习裁剪外加入通道缩放类结构变换，增强低比特 W/A 量化精度。
算法详细介绍请参见：[OmniQuant: Omnidirectionally Calibrated Quantization for Large Language Models](https://arxiv.org/abs/2308.13137) 。

## 激活量化算法

### LAC

LAC（Learnable Activation Clipping，可学习激活裁剪范围），是面向神经网络量化（尤其LLM）的可微激活范围优化技术，在可学习裁剪外加入通道缩放类结构变换，增强低比特 W/A 量化精度。算法详细介绍请参见：[OmniQuant: Omnidirectionally Calibrated Quantization for Large Language Models](https://arxiv.org/abs/2308.13137)。
...
![](./figures/histogram_merg_process.png "直方图合并过程")

**图 2**  搜索得到最优的左右截断位置<a name="fig1419882412"></a>  
![](./figures/clip_min_max_search.png "搜索得到最优的左右截断位置")
...
为获得最佳量化效果，可以不断改变截断范围\[clip\_min, clip\_max\]，选择量化效果最好的一组范围作为最终的量化结果。IFMR算法中，clip\_min和clip\_max用来设置搜索范围和搜索步长，遍历查询得到最优的量化效果。量化配置中FMRQuantize（以PyTorch框架为例，参数解释请参见[表1](./context/ptq_config.md#zh-cn_topic_0240188735_table1225503375617)）提供的参数则用来调整截断范围。
...
OmniQuant是面向LLM的轻量级、可微分训练后量化算法，核心是风格结构变换，学习通道缩放：在可学习裁剪外加入通道缩放类结构变换，增强低比特W/A量化精度。
...
算法详细介绍请参见： [SmoothQuant: Accurate and Efficient Post-Training Quantization for Large Language Models](https://arxiv.org/abs/2211.10438)。

### FlatQuant

FlatQuant 是一种面向大模型的训练后量化（PTQ）方法，核心思想是通过可学习的仿射变换对激活和权重进行数学等效变换，使激活分布更加平坦，从而降低低比特量化难度。具体地，FlatQuant 引入可学习的仿射变换、对角缩放以及 Kronecker 分解矩阵等结构化参数，在保持线性层计算等价性的同时，对 hidden 维度上的激活和权重进重新分布，使异常值在更多维度上被摊平。一般来说，激活分布越尖锐、异常值越集中，FlatQuant 的结构变换越有助于改善量化后的精度表现。

算法详细介绍请参见： [FlatQuant: Flatness Matters for LLM Quantization](https://arxiv.org/abs/2410.09426)。

### ULQ数据量化算法

该算法用于量化感知训练场景。

ULQ（Universal Linear Quantization）算法在训练过程中不断训练量化因子，以减少量化损失。该算法初始化时会对数值做量化（包含截断），对初始化敏感。量化配置中ULQuantize（以PyTorch框架为例，参数解释请参见[量化感知训练简易配置文件](./context/qat_config.md)来控制ULQ算法。
...
![](./figures/channel_sparsity_algorithm.png "通道稀疏算法原理")

稀疏配置文件中通过BalancedL2NormFilterPruner字段（以PyTorch框架为例，参数解释请参见[量化感知训练简易配置文件](./context/qat_config.md)）来控制BalancedL2Norm算法。
...
稀疏配置文件中通过L1SelectivePruner字段（参数解释请参见[量化感知训练简易配置文件](./context/qat_config.md)）来控制L1SelectivePrune算法。
...
4. 完成所有的模块蒸馏，得到精度更优的量化模型。
   ...
   **图 1**  蒸馏示意图<a name="fig275521214542"></a>  
   ![](./figures/distill_algorithm.png "蒸馏示意图")
