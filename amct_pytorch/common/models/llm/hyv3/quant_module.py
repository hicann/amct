...
class QuantHYV3MoE(nn.Module):
    def __init__(self, args, moe_module: HYV3MoE):
        ...
        self.register_buffer(
            "e_score_correction_bias", moe_module.e_score_correction_bias
        )
        self.enable_moe_fp32_combine = moe_module.enable_moe_fp32_combine
        shared_args = build_no_algo_args(args)
        self.shared_experts = QuantHYV3MLP(
            shared_args, moe_module.shared_experts, group="moe.shared"
        )
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.enable_moe_fp32_combine:
            hidden_states = (
                routed_output.float() + self.shared_experts(hidden_states).float()
