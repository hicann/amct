...
from amct_pytorch.algorithms import BUILT_IN_ALGORITHM
...
def check_config(quant_data_comb, quant_config, algo):
    ...
    if quant_data_comb not in ALGORITHM_SUPPORTED_QUANT_TYPE_COMB.keys():
        raise ValueError(
            f"Do not support combination {quant_data_comb} of act and weight quant dtype."
        )
    if algo not in ALGORITHM_SUPPORTED_QUANT_TYPE_COMB[quant_data_comb]:
        raise ValueError(
            f"Algorithm {algo} do not support act and weight quant dtype {quant_data_comb}"
        )
    ...
    if quant_data_comb not in WTS_GRANULARITY_SUPPORT_MAP.get(weight_strategy):
        raise ValueError(
            f"act_dtype and wts_dtype {quant_data_comb} do not support weight "
    ...
    if quant_config.quant_cfg.inputs_cfg.quant_input:
        ...
        if act_dtype not in [
            "mxfp8_e4m3fn"
        ] and quant_data_comb not in ACT_GRANULARITY_SUPPORT_MAP.get(act_strategy):
            raise ValueError(
                f"act_dtype and wts_dtype {quant_data_comb} do not support "
                f"activation quant strategy {act_strategy}"
            )
        if act_dtype in ['mxfp8_e4m3fn'] and act_strategy != 'group':
            raise ValueError(
                f"act_dtype and wts_dtype {quant_data_comb} only support activation quant strategy group"
            )
    ...
    if quant_config.quant_cfg.weights_cfg.group_size is not None:
        ...
        if group_size < 32 or group_size % 32 != 0:
            raise ValueError(
                f"act_type and wts_type {quant_data_comb} only support group_size larger than 32 and "
...
def _check_quant_dtype_comb_rules(quant_data_comb, quant_config):
    ...
    if (
        ...
        and weight_cfg.symmetric is False
    ):
        raise ValueError(
            f"{quant_data_comb} only support symmetric weight quantization"
        )
    ...
    if weight_cfg.strategy not in ['tensor', 'channel']:
        raise ValueError(
            f"{quant_data_comb} only support weight quant strategy tensor or channel"
        )
    if inputs_cfg.strategy != 'tensor':
        raise ValueError(
            f"{quant_data_comb} only support activation quant strategy tensor"
        )
...
def _check_fuzzy_config_warnings(all_layer_names, quant_config):
    ...
    for fuzzy_cfg in all_fuzzy_patterns:
        ...
        for layer_name in all_layer_names:
            ...
            if layer_name in skip_layers_set:
                LOGGER.logw(
                    f"Fuzzy matched layer {layer_name} is also in skip_layers, "
                    f"quantization will be skipped",
                    "ConfigParser",
                )
    ...
    for pattern, match_count in pattern_match_count.items():
        if match_count == 0:
            LOGGER.logw(
                f"Fuzzy pattern {pattern} does not match any layer in the model",
                "ConfigParser",
            )
...
def check_kv_config(quant_config, algo):
    ...
    if algo not in ALGORITHM_SUPPORTED_QUANT_TYPE_KV_CACHE[quant_type]:
        raise ValueError(
            f"Algorithm {algo} do not support kvcache quant dtype {quant_type}"
        )
...
def _is_layer_supported(mod, name, layer_types, quant_type_comb, quant_config):
    ...
    if type(mod).__name__ == 'FP8Linear':
        ...
        return False
    if (
        type(mod) not in layer_types.keys()
        and type(mod).__name__ not in layer_types.keys()
    ):
        return False
    if (
        quant_config.quant_cfg.kvcache_cfg.quant_kv
        and type(mod).__name__ not in KVCACHE_OPS
    ):
        return False
    if type(mod).__name__ == 'Conv2d' and mod.padding_mode != 'zeros':
        LOGGER.logd(
            "layer:{} cannot be quantized, act_dtype and wts dtype {} has requirement "
            "on padding_mode of conv2d, padding_mode should be zero.".format(
                name, quant_type_comb
            )
        )
        return False
...
def get_supported_layers(model, quant_config, registed_alg):
    ...
    for name, mod in model.named_modules():
        if not _is_layer_supported(
            mod, name, layer_types, quant_type_comb, quant_config
        ):
            continue
        ...
        detail_config_algo = {
            "algorithm": {algo: quant_config.algorithm.get_value()["algorithm"][algo]}
        }
        detail_config[name] = (
            quant_config.batch_num.get_value() | layer_quant_cfg | detail_config_algo
        )
