...
from amct_pytorch.common.utils.vars import (
    SUPPORT_QUANT_STRATEGY_WEIGHT,
    SUPPORT_QUANT_STRATEGY_INPUT,
)
from amct_pytorch.common.utils.vars import (
    SUPPORT_QUANT_DYNAMIC_INPUT,
    SUPPORT_KVCACHE_QUANT_DTYPE,
)
from amct_pytorch.common.utils.vars import (
    WTS_ASYMMETRIC_DTYPE,
    GROUP_SIZE_SUPPORTED_DTYPE,
)
from amct_pytorch.common.utils.vars import GROUP_SIZE_SUPPORTED_MAP
...
class BatchNumField:
    ...
    def check(self):
        if not isinstance(self.batch_num, int) or self.batch_num <= 0:
            raise ValueError(
                f'Batch num only support positive int, but got {self.batch_num}'
            )
...
class WeightsCfgField:
    ...
    def check(self):
        if self.quant_type not in SUPPORT_WEIGHT_QUANT_DTYPE:
            raise ValueError(
                f'Weights quant_dtype only support {SUPPORT_WEIGHT_QUANT_DTYPE}, '
        ...
        if self.strategy not in SUPPORT_QUANT_STRATEGY_WEIGHT:
            raise ValueError(
                f'Weights strategy only support{SUPPORT_QUANT_STRATEGY_WEIGHT}, but got {self.strategy}'
            )
        if self.group_size is not None and self.strategy != 'group':
            raise ValueError(
                f'Weights group_size only support strategy group, but got {self.strategy}'
            )
    ...
    def check_group_size(self):
        ...
        if not isinstance(self.group_size, int) or self.group_size <= 0:
            raise ValueError(
                f'group_size only support positive int, but got {self.group_size}'
            )
        if self.quant_type not in GROUP_SIZE_SUPPORTED_DTYPE:
            raise ValueError(
                f'Weights group_size only support for '
                f'weight dtype: {GROUP_SIZE_SUPPORTED_DTYPE}'
            )
        if (
            self.quant_type in GROUP_SIZE_SUPPORTED_MAP.keys()
            and self.group_size not in GROUP_SIZE_SUPPORTED_MAP.get(self.quant_type)
        ):
            raise ValueError(
                f'wts_type {self.quant_type} only support group_size value: '
...
class InputsCfgField:
    ...
    def check(self):
        if self.quant_type not in SUPPORT_INPUT_QUANT_DTYPE:
            raise ValueError(
                f'Inputs quant_dtype only support {SUPPORT_INPUT_QUANT_DTYPE}, but got {self.quant_type}'
            )
        if self.symmetric not in [True, False]:
            raise ValueError(
        ...
        if (
            self.strategy not in SUPPORT_QUANT_STRATEGY_INPUT
            and self.quant_type not in ['mxfp8_e4m3fn']
        ):
            raise ValueError(
        ...
        if (
            ...
            and self.strategy not in SUPPORT_QUANT_DYNAMIC_INPUT
        ):
            raise ValueError(
                f'Inputs dynamic only support strategy {SUPPORT_QUANT_DYNAMIC_INPUT}, '
...
class KvcacheCfgField:
    ...
    def check(self):
        if self.quant_type not in SUPPORT_KVCACHE_QUANT_DTYPE:
            raise ValueError(
                f'Kvcache quant_dtype only support {SUPPORT_KVCACHE_QUANT_DTYPE}, " \
                            "but got {self.quant_type}'
            )
        if self.symmetric not in [True]:
            raise ValueError(
                f'Kvcache symmetric only support bool [True], but got {self.symmetric}'
            )
        if self.strategy != 'tensor':
            raise ValueError(
                f'Kvcache strategy only support tensor, but got {self.strategy}'
            )
...
class QuantCfgField:
    def __init__(self, config):
        ...
        self.kvcache_cfg = KvcacheCfgField(
            config.get('kvcache', {'enable_quant': False})
        )
        ...
        for key, value in config.items():
            if '*' in key:
                if key.endswith('.weights'):
                    fuzzy_configs['weights'].append({'pattern': key, 'config': value})
                elif key.endswith('.inputs'):
                    fuzzy_configs['inputs'].append({'pattern': key, 'config': value})
                elif key.endswith('.kvcache'):
                    fuzzy_configs['kvcache'].append({'pattern': key, 'config': value})

        return fuzzy_configs

    def set_value(self):
        return {
            'weights_cfg': self.weights_cfg.get_value(),
...
class AwqField:
    ...
    def check(self):
        if self.attrs is None or self.attrs.get('grids_num') is None:
            raise ValueError('Awq grids_num is necessary param, pls check and set')
        self.grids_num = self.attrs.get('grids_num')
        if not isinstance(self.grids_num, int) or self.grids_num <= 0:
            raise ValueError(
                f'Awq grids_num only support positive int, but got {self.grids_num}'
            )
...
class SmoothQuantField:
    ...
    def check(self):
        if self.attrs is None or self.attrs.get('smooth_strength') is None:
            raise ValueError(
                'Smoothquant smooth_strength is necessary param, pls check and set'
            )
        self.smooth_strength = self.attrs.get('smooth_strength')
        if (
            not isinstance(self.smooth_strength, float)
            or self.smooth_strength <= 0
            or self.smooth_strength >= 1
        ):
            raise ValueError(
                f'Smoothquant smooth_strength only support float (0, 1), but got {self.smooth_strength}'
            )
...
class AlgorithmField:
    def __init__(self, alg_cfg, alg_reg):
        ...
        assert len(alg_names) == len(alg_attrs), (
            "alg_names and alg_attrs should have the same length"
        )
        ...
        for alg_name, alg_attr in zip(alg_names, alg_attrs):
            for src_op in AlgorithmRegistry.algo.get(alg_name).keys():
                if src_op in supported_src_op:
                    raise ValueError(
                        f'One src_op only support one algorithm, current algo {alg_name} '
    ...
    def check(self):
        for layer in self.skip_layers:
            if not isinstance(layer, str):
                raise ValueError('Skip layers must be str')
