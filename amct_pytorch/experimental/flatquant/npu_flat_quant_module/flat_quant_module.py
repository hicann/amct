...
class NpuFlatQuantAttention(torch.nn.Module):
    def __init__(self, module_config, layer_idx, quant_config):
        ...
        if self.quant_config.use_o_quant:
            self.register_parameter(
                'scale_o_proj', torch.nn.Parameter(requires_grad=False)
            )
            self.register_parameter(
                'wt_o_packed', torch.nn.Parameter(requires_grad=False)
            )
    ...
    def from_quant_module(cls, quant_module) -> 'NpuFlatQuantAttention':
        ...
        if type(quant_module).__name__ != 'FlatQuantAttention':
            raise ValueError(
                f'not support construct "NpuFlatQuantAttention" from module {type(quant_module).__name__}'
            )

        flat_attn = cls(
            quant_module.module_config, quant_module.layer_idx, quant_module.flat_config
        )

        if hasattr(quant_module, 'sliding_window'):
            flat_attn.sliding_window = quant_module.sliding_window
        flat_attn.q_norm = (
            quant_module.q_norm if hasattr(quant_module, 'q_norm') else None
        )
        flat_attn.k_norm = (
            quant_module.k_norm if hasattr(quant_module, 'k_norm') else None
        )

        quant_module.q_proj.weight_quantizer.find_params(
            quant_module.q_proj.linear.weight
        )
        quant_module.k_proj.weight_quantizer.find_params(
            quant_module.k_proj.linear.weight
        )
        quant_module.v_proj.weight_quantizer.find_params(
            quant_module.v_proj.linear.weight
        )
        ...
        flat_attn.wt_q_packed = torch.nn.Parameter(
            pack_int4_weights(wt_q_proj.data), requires_grad=False
        )
        flat_attn.wt_k_packed = torch.nn.Parameter(
            pack_int4_weights(wt_k_proj.data), requires_grad=False
        )
        flat_attn.wt_v_packed = torch.nn.Parameter(
            pack_int4_weights(wt_v_proj.data), requires_grad=False
        )
        flat_attn.q_lac = torch.nn.Parameter(
            quant_module.q_proj.lac_ratio, requires_grad=False
        )
        flat_attn.k_lac = torch.nn.Parameter(
            quant_module.k_proj.lac_ratio, requires_grad=False
        )
        flat_attn.v_lac = torch.nn.Parameter(
            quant_module.v_proj.lac_ratio, requires_grad=False
        )
        flat_attn.left_trans = torch.nn.Parameter(
            quant_module.ln_trans.matrix_left.T, requires_grad=False
        )
        flat_attn.right_trans = torch.nn.Parameter(
            quant_module.ln_trans.matrix_right, requires_grad=False
        )

        if quant_module.flat_config.use_o_quant:
            quant_module.o_proj.weight_quantizer.find_params(
                quant_module.o_proj.linear.weight
            )
            wt_o_proj, scale_o_proj = quant_module.o_proj.weight_quantizer.quantize(
                quant_module.o_proj.linear.weight, quantonly=True
            )
            flat_attn.scale_o_proj = torch.nn.Parameter(
                scale_o_proj, requires_grad=False
            )
            flat_attn.wt_o_packed = torch.nn.Parameter(
                pack_int4_weights(wt_o_proj.data), requires_grad=False
            )
            flat_attn.o_trans = torch.nn.Parameter(
                quant_module.o_trans.get_matrix().T, requires_grad=False
            )
        else:
    ...
    def forward(self, hidden_states, **kwargs):
        ...
        if bsz % 16 and q_len % 16:
            ...
            padding = (
                (0, 0) * (hidden_states.dim() - 2) + (0, q_len_pad_size_after) + (0, 0)
            )
    ...
    def forward_qkv(self, query_states, key_states, value_states, bsz, q_len, **kwargs):
        ...
        attn_implementation = getattr(self.config, '_attn_implementation')
        if attn_implementation != 'eager':
            if attn_implementation == 'sdpa' and kwargs.get('output_attentions', False):
                print(
        ...
        if self.quant_config.use_o_quant:
            ...
            attn_output = torch.matmul(
                self.o_trans.to(attn_output), attn_output
            ).reshape(init_shape)
...
class NpuFlatQuantMLP(torch.nn.Module):
    def __init__(self, config, quant_config):
        ...
        self.register_parameter(
            'scale_up_proj', torch.nn.Parameter(requires_grad=False)
        )
        self.register_parameter(
            'scale_gate_proj', torch.nn.Parameter(requires_grad=False)
        )
        ...
        self.register_parameter(
            'wt_gate_packed', torch.nn.Parameter(requires_grad=False)
        )
        # flat quant trans matrix
        self.register_parameter(
            'ug_left_trans', torch.nn.Parameter(requires_grad=False)
        )
        self.register_parameter(
            'ug_right_trans', torch.nn.Parameter(requires_grad=False)
        )

        if self.quant_config.use_down_quant:
            self.register_parameter(
                'scale_down_proj', torch.nn.Parameter(requires_grad=False)
            )
            self.register_parameter(
                'wt_down_packed', torch.nn.Parameter(requires_grad=False)
            )
            self.register_parameter(
                'down_left_trans', torch.nn.Parameter(requires_grad=False)
            )
            self.register_parameter(
                'down_right_trans', torch.nn.Parameter(requires_grad=False)
            )
        else:
    ...
    def from_quant_module(cls, quant_module) -> 'NpuFlatQuantMLP':
        ...
        if type(quant_module).__name__ != 'FlatQuantMLP':
            raise ValueError(
                f'not support construct "NpuFlatQuantMLP" from module {type(quant_module).__name__}'
            )

        flat_mlp = cls(quant_module.module_config, quant_module.flat_config)
        flat_mlp.act_fn = quant_module.act_fn

        quant_module.up_proj.weight_quantizer.find_params(
            quant_module.up_proj.linear.weight
        )
        quant_module.gate_proj.weight_quantizer.find_params(
            quant_module.gate_proj.linear.weight
        )

        wt_up_proj, scale_up_proj = quant_module.up_proj.weight_quantizer.quantize(
            quant_module.up_proj.linear.weight, quantonly=True
        )
        flat_mlp.wt_up_packed = torch.nn.Parameter(
            pack_int4_weights(wt_up_proj.data), requires_grad=False
        )
        flat_mlp.scale_up_proj = torch.nn.Parameter(scale_up_proj, requires_grad=False)

        wt_gate_proj, scale_gate_proj = (
            quant_module.gate_proj.weight_quantizer.quantize(
                quant_module.gate_proj.linear.weight, quantonly=True
            )
        )
        flat_mlp.wt_gate_packed = torch.nn.Parameter(
            pack_int4_weights(wt_gate_proj.data), requires_grad=False
        )
        flat_mlp.scale_gate_proj = torch.nn.Parameter(
            scale_gate_proj, requires_grad=False
        )
        flat_mlp.ug_left_trans = torch.nn.Parameter(
            quant_module.up_gate_trans.matrix_left.T, requires_grad=False
        )
        flat_mlp.ug_right_trans = torch.nn.Parameter(
            quant_module.up_gate_trans.matrix_right, requires_grad=False
        )
        flat_mlp.up_lac = torch.nn.Parameter(
            quant_module.up_proj.lac_ratio, requires_grad=False
        )
        flat_mlp.gate_lac = torch.nn.Parameter(
            quant_module.gate_proj.lac_ratio, requires_grad=False
        )

        if flat_mlp.quant_config.use_down_quant:
            quant_module.down_proj.weight_quantizer.find_params(
                quant_module.down_proj.linear.weight
            )
            wt_down_proj, scale_down_proj = (
                quant_module.down_proj.weight_quantizer.quantize(
                    quant_module.down_proj.linear.weight, quantonly=True
                )
            )
            flat_mlp.wt_down_packed = torch.nn.Parameter(
                pack_int4_weights(wt_down_proj.data), requires_grad=False
            )
            flat_mlp.scale_down_proj = torch.nn.Parameter(
                scale_down_proj, requires_grad=False
            )
            flat_mlp.down_left_trans = torch.nn.Parameter(
                quant_module.down_trans.matrix_left.T, requires_grad=False
            )
            flat_mlp.down_right_trans = torch.nn.Parameter(
                quant_module.down_trans.matrix_right, requires_grad=False
            )
            flat_mlp.down_lac = torch.nn.Parameter(
                quant_module.down_proj.lac_ratio, requires_grad=False
            )
        else:
    ...
    def forward(self, x):
        ...
        if self.quant_config.use_down_quant:
            down_states = self.method.apply(
                self.down_left_trans,
                self.down_right_trans,
                self.wt_down_packed,
                self.scale_down_proj,
                x_act_fn,
                self.down_lac.data,
            )
        else:
