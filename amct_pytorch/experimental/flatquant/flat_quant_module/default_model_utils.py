...
def flat_config_parser(layer_config):
    ...
    # quant config
    flat_config.a_bits = int(
        layer_config.get('inputs_cfg').get('quant_type').replace('int', '')
    )
    flat_config.w_bits = int(
        layer_config.get('weights_cfg').get('quant_type').replace('int', '')
    )
    ...
    flat_config.use_o_quant = flatquant_config.get(
        'use_o_quant', FlatConfig.use_o_quant
    )
    flat_config.use_down_quant = flatquant_config.get(
        'use_down_quant', FlatConfig.use_down_quant
    )

    # kv config
    flat_config.use_kcache_quant = flatquant_config.get(
        'use_kcache_quant', FlatConfig.use_kcache_quant
    )
    ...
    flat_config.use_vcache_quant = flatquant_config.get(
        'use_vcache_quant', FlatConfig.use_vcache_quant
    )
...
class FlatQuantAttention(nn.Module):
    def __init__(self, module: nn.Module, layer_name: str = "", layer_config=None):
        ...
        self.q_proj = FlatQuantizedLinear(
            module.q_proj, self.flat_config, self.flat_config.lac
        )
        self.k_proj = FlatQuantizedLinear(
            module.k_proj, self.flat_config, self.flat_config.lac
        )
        self.v_proj = FlatQuantizedLinear(
            module.v_proj, self.flat_config, self.flat_config.lac
        )
        ...
        if self.flat_config.use_kcache_quant and self.flat_config.k_bits < 16:
            self.k_cache_quantizer = ActivationQuantizer(
                bits=self.flat_config.k_bits,
                sym=self.flat_config.k_sym,
                lac=self.flat_config.lac,
            )
        if self.flat_config.use_vcache_quant and self.flat_config.v_bits < 16:
            self.v_cache_quantizer = ActivationQuantizer(
                bits=self.flat_config.v_bits,
                sym=self.flat_config.v_sym,
                lac=self.flat_config.lac,
            )
        ...
        self.ln_smax = (
            torch.ones_like(self.q_proj.linear.weight.abs().max(dim=0)[0]).npu() * 1e-5
        )
    ...
    def add_fq_trans(self):
        ...
        if self.flat_config.a_bits < 16 or self.flat_config.w_bits < 16:
            ...
            self.ln_trans = SVDDecomposeTransMatrix(
                ln_dim_left, ln_dim_right, add_diag=self.flat_config.add_diag
            )
        if self.flat_config.use_o_quant and (
            self.flat_config.a_bits < 16 or self.flat_config.w_bits < 16
        ):
        ...
        if (
            self.flat_config.use_vcache_quant
            and self.flat_config.v_bits < 16
            and self.vcache_trans is None
        ):
            self.vcache_trans = SVDSingleTransMatrix(self.head_dim)
    ...
    def forward_qkv(self, query_states, key_states, value_states, bsz, q_len, **kwargs):
        ...
        attn_implementation = getattr(self.module_config, '_attn_implementation')
...
class FlatQuantMLP(nn.Module):
    def __init__(self, module: nn.Module, layer_name: str = "", layer_config=None):
        ...
        self.up_proj = FlatQuantizedLinear(
            module.up_proj, self.flat_config, self.flat_config.lac
        )
        self.gate_proj = FlatQuantizedLinear(
            module.gate_proj, self.flat_config, self.flat_config.lac
        )
        self.down_proj = FlatQuantizedLinear(
            module.down_proj, self.flat_config, self.flat_config.lac
        )
        self.add_fq_trans()

        self._ori_mode = False
        self.up_smax = (
            torch.ones_like(self.up_proj.linear.weight.abs().max(dim=0)[0]).npu() * 1e-5
        )
        self.down_smax = (
            torch.ones_like(self.down_proj.linear.weight.abs().max(dim=0)[0]).npu()
            * 1e-5
        )
    ...
    def add_fq_trans(self):
        if self.flat_config.a_bits < 16 or self.flat_config.w_bits < 16:
            ...
            self.up_gate_trans = SVDDecomposeTransMatrix(
                up_dim_left, up_dim_right, add_diag=self.flat_config.add_diag
            )
            ...
            if self.flat_config.use_down_quant:
                ...
                self.down_trans = SVDDecomposeTransMatrix(
                    down_dim_left, down_dim_right, add_diag=self.flat_config.add_diag
                )
            else:
    ...
    def _ori_forward(self, x):
        ...
        self.up_smax = torch.maximum(
            self.up_smax, x.reshape(-1, x.shape[-1]).abs().max(0)[0].clone().detach()
        )
        x = self.act_fn(self.gate_proj._ori_forward(x)) * self.up_proj._ori_forward(x)
        self.down_smax = torch.maximum(
            self.down_smax, x.reshape(-1, x.shape[-1]).abs().max(0)[0].clone().detach()
        )
