...
def generate_li_hadamard_matrix(num_layers, dim=128):
    hadamard_matrixs = {}
...
def load_clip_params(num_hidden_layers, num_nextn_predict_layers, mla_param_path, moe_param_path):
    ...
    act_clip_params = {}
    weight_clip_params = {}
    clip_param_files = list(glob(os.path.join(mla_param_path, "*.pth")))
...
def convert_clip_factors(block_id, merge_quant_param):
    ...
    with torch.no_grad():
        save_clip_factor_param = {}
        for key, val in merge_quant_param.items():
            new_key = None
            if "max" in key:
                if "k_cache_quantizer.clip_factor_a_max" in key:
                    ...
                    if block_id <= 2:
                        ...
                        val.data = torch.tensor(10.0)
                if new_key:
                    factor_param = torch.sigmoid(val)
