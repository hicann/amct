...
class QuantIndexer(torch.nn.Module):
    def __init__(self, module, args, rotary_emb, bit):
        ...
        self.wk = Linear(
            self.dim, self.head_dim, dtype=torch.get_default_dtype(), bias=False
        )
        self.k_norm = LayerNorm(self.head_dim)
        self.weights_proj = Linear(
            self.dim, self.n_heads, dtype=torch.get_default_dtype(), bias=False
        )
        self.softmax_scale = self.head_dim**-0.5
        self.scale_fmt = args.model_args.scale_fmt
        self.rotary_emb = rotary_emb
        self.afq_rot_q = ActivationQuantizer(
            bits=self.bit, sym=True, lac=False, groupsize=-1, per_tensor=False
        )
        self.afq_rot_k = ActivationQuantizer(
            bits=self.bit, sym=True, lac=False, groupsize=-1, per_tensor=False
        )
...
class QuantDSA(DeepseekV3Attention):
    def __init__(self, args, module: DeepseekV3Attention):
        ...
        self.q_cache_quantizer = ActivationQuantizer(
            bits=16, sym=not (args.q_asym), lac=False, groupsize=-1
        )
        ...
        self.afq1 = ActivationQuantizer(
            bits=self.bit, sym=True, lac=False, groupsize=-1, per_tensor=False
        )
        self.afq2 = ActivationQuantizer(
            bits=self.bit, sym=True, lac=False, groupsize=-1, per_tensor=False
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.q_lora_rank is not None:
            ...
            q_b_proj = self.q_b_proj.weight.t().reshape(
                q_b_proj_inc, self.num_heads, self.q_head_dim
            )
            q_b_proj_c_weight, q_b_proj_r_weight = torch.split(
                q_b_proj, [self.qk_nope_head_dim, self.qk_rope_head_dim], dim=-1
            )

        # get Wdkv and Wkr
        dkv_weight, kr_weight = torch.split(
            self.kv_a_proj_with_mqa.weight.t(),
        ...
        ukv_weight = self.kv_b_proj.weight.t().reshape(
            kv_b_proj_inc, self.num_heads, (self.qk_nope_head_dim + self.v_head_dim)
        )
        uk_weight, uv_weight = torch.split(
            ukv_weight, [self.qk_nope_head_dim, self.v_head_dim], dim=-1
        )
        uk_weight = (
            uk_weight.reshape(kv_b_proj_inc, -1)
            .t()
            .reshape(self.num_heads, self.qk_nope_head_dim, kv_b_proj_inc)
        )
        ...
        if self.q_lora_rank is not None:
            # wqr
            qr = (
                self.qr_matmul(q, q_b_proj_r_weight.reshape(q_b_proj_inc, -1))
                .view(bsz, q_len, self.num_heads, self.qk_rope_head_dim)
                .transpose(1, 2)
            )
            # wuq
            q_nope = (
                self.qnope_matmul(q, q_b_proj_c_weight.reshape(q_b_proj_inc, -1))
        ...
        if attention_mask is not None:
            ...
            topk_indices = self.indexer(
                hidden_states, q_norm, attention_mask, position_ids
            )
            index_mask = (
                torch.full(
                    (bsz, kv_seq_len, kv_seq_len),
                    float("-inf"),
        ...
        attn_output = torch.matmul(
            attn_weights, fix_k_c_r.squeeze(1)[:, :, :kv_b_proj_inc]
        )
