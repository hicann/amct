...
from cores.calibrator.function_utils import (
...
def forward_once(layer, fp_inps, seq_length, index, args):
    batch_slice = slice(index, index + args.cali_bsz)
    if args.train_mode == "mla":
        ...
        out = layer(fp_inps[batch_slice], attention_mask=attention_mask_batch)[0]
    else:
        out = layer(fp_inps[batch_slice])
    return out
...
def cali_quant(args, layer, layer_idx, inps, dev, param_prefix=None):
    ...
    with torch.no_grad():
        for j in range(args.nsamples // args.cali_bsz):
            ...
            batch_slice = slice(index, index + args.cali_bsz)
            fp_outs[batch_slice] = forward_once(layer, fp_inps, seq_length, index, args)
    ...
    for epoch in range(args.epochs):
        ...
        logger.info(
            f"lwc lac iter {epoch}, lr {cur_lr:.8f} time {time.time() - start_tick:.6f}s, mse: {mse:.8f}"
        )
    ...
    for name, mod in layer.named_modules():
        ...
        if isinstance(mod, ActivationQuantizer):
            ...
            if hasattr(mod, 'clip_factor_a_min'):
                quant_parameters[f"{mod_name}.clip_factor_a_min"] = (
                    mod.clip_factor_a_min
                )
                quant_parameters[f"{mod_name}.clip_factor_a_max"] = (
                    mod.clip_factor_a_max
                )
        if isinstance(mod, QuantLinear):
            if hasattr(mod, 'clip_factor_w_max'):
                quant_parameters[f"{mod_name}.clip_factor_w_max"] = (
                    mod.clip_factor_w_max
                )
                quant_parameters[f"{mod_name}.clip_factor_w_min"] = (
                    mod.clip_factor_w_min
                )
