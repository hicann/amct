...
class NpuHIF8CastLinear(torch.nn.Module):
    ...
    def __init__(self, quant_module):
        ...
        weight, _ = quant_tensor(
            quant_module.weight.transpose(1, 0),
            quant_module.wts_type,
            scale=self.scale_w,
        )
    ...
    def forward(self, x):
        ...
        if self.weight_compress_only:
            ...
            LOGGER.logd(
                f"Cast activation of layer '{self.layer_name}' to {HIFLOAT8} success",
                'NpuHIF8CastLinear',
            )
        return output
