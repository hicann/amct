...
class NpuHIF8Linear(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        quant_x, pertoken_scale = torch_npu.npu_dynamic_quant(
            x, dst_type=torch_npu.hifloat8, dst_type_max=15
        )
        ...
        if self.is_support_hifloat8:
            ...
            raise RuntimeError('current environment do not support hifloat8 quantize')
    ...
    def _init_weight_quant(self, quant_module):
        ...
        # k, n * n
        hif8_quantized_tensor = torch_npu.npu_quantize(
            dequant_weight.transpose(0, 1), scale, None, dtype=torch_npu.hifloat8
        )
