...
class DeepseekV3AttentionQuant(BaseQuantizeModule):
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Function: DeepseekV3AttentionQuant forward function.
        Args:
        ...
        self.previous_max_k = (
            batch_max_k
            if self.previous_max_k is None
            else calculate_quantile_ema_scale(self.previous_max_k, batch_max_k)
        )
        self.previous_max_v = (
            batch_max_v
            if self.previous_max_v is None
            else calculate_quantile_ema_scale(self.previous_max_v, batch_max_v)
        )
        if self.cur_batch == self.batch_num:
    ...
    def _hook_kv_states(self, past_key_value):
        ...
        def hook_update(key_states, value_states, layer_idx, **kwargs):
            ...
            LOGGER.logd(
                "Capture data from layer '{}' success!".format(self.layer_name),
                'DeepseekV3AttentionQuant',
            )
