    ...
    def do_pass(self, graph, object_node, model=None):
        ...
        if len(object_node.output_anchors) == 1:
            ...
            if len(output_anchor.get_peer_input_anchor()) == 1:
                ...
                if (
                    next_node.type == "BatchNormalization"
                    and object_node.type == "Conv"
                ):
        ...
        if object_node.type in RNN_LAYER_TYPE:
            ...
            new_module = RetrainQuant(
                object_module, self.record_module, bn_module, bn_module_name
            )
        setattr(parent_module, object_node_name.split('.')[-1], new_module)
        if is_next_node_bn:
            bn_parent_module = model_helper.get_parent_module(next_node.name)
            setattr(
                bn_parent_module, next_node.name.split(".")[-1], torch.nn.Identity()
            )

        LOGGER.logd(
            "Insert RetrainQuant module to '{}' success!".format(object_node.name),
