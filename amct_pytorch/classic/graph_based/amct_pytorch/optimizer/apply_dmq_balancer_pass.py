...
class ApplyDMQBalancerPass(BaseFusionPass):
    ...
    @staticmethod
    def _construct_factor_node(
        layer_name, tensor_balance_factor, factor_data_type, reshape_size
    ):
        ...
        TensorProtoHelper(attr.t).set_data(
            tensor_balance_factor, factor_data_type, reshape_size
        )
    ...
    def _construct_mul_node(layer_name):
        ...
        node_proto.output.extend(['.'.join([layer_name, MUL, 'output0'])])
    ...
    def do_pass(self, graph, object_node, model=None):
        ...
        tensor_balance_factor_node = graph.add_node(
            ApplyDMQBalancerPass._construct_factor_node(
        ...
        LOGGER.logd(
            "Insert dmq_balancer layer '{}' before '{}' success!".format(
                mul_node.name, object_name
            ),
