...
from ...amct_pytorch.custom_op.comp_module.comp_module_conv_transpose1d import (
    CompModuleConvTranspose1d,
)
from ...amct_pytorch.custom_op.comp_module.comp_module_conv_transpose2d import (
    CompModuleConvTranspose2d,
)
from ...amct_pytorch.custom_op.comp_module.comp_module_conv_transpose3d import (
    CompModuleConvTranspose3d,
)
...
class InsertRetrainPass(BaseFusionPass):
    ...
    def do_pass(self, graph, object_node, model=None):
        ...
        wts_config = self.conf.get_retrain_config(object_node_name)[
        ...
        common_config = {
            ...
            'fakequant_precision_mode': self.conf.retrain_config.get(
    ...
    def _get_act_config(self, object_node):
        ...
        """
        act_config = self.conf.get_retrain_config(object_node.name)[
            'retrain_data_config'
        ]
        act_config['ifmr_init'] = False
        if act_config.get('clip_min') is None or act_config.get('clip_max') is None:
        ...
        h_pre_node = (
            object_node.get_input_anchor(h_index).get_peer_output_anchor().node
        ...
        act_config[NUM_BITS] = NUM_BITS_MAP.get(
            act_config.get('dst_type'), DEFAULT_NUM_BITS
        )
