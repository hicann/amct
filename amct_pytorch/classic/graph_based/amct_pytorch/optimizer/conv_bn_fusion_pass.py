...
class ConvBnFusionPass(BaseModuleFusionPass):
    ...
    def is_fusionable_conv(conv_module, conv_name):
        ...
        if conv_module.padding_mode != 'zeros':
            LOGGER.logd(
                'Can only do Conv+BN fusion of Conv that padding_mode '
                '= "zeros", but "%s" is "%s"' % (conv_name, conv_module.padding_mode),
                'ConvBnFusionPass',
            )
            return False
        return True
    ...
    def set_fused_data_back(fused_conv_module, conv_node, graph):
        ...
        if len(conv_node.input_anchors) <= bias_index:
            ...
            bias_helper.set_data(
                fused_bias_raw, type_string=data_type, dims=[fused_bias_tensor.numel()]
            )
    ...
    def match_pattern(self, module, name, graph=None):
        ...
        if not isinstance(
            ...
            return False
        if (
            isinstance(module, (torch.nn.BatchNorm3d, torch.nn.BatchNorm1d))
            and torch.__version__ != LOW_TORCH_VERSION
            and not version_higher_than(torch.__version__, LOW_TORCH_VERSION)
        ):
            return False
        ...
        if len(bn_node.input_anchors) != 5:
            raise RuntimeError(
                'BatchNorm node should only have 5 input actually have {}'.format(
                    len(bn_node.input_anchors)
                )
            )
        ...
        if (
            len(conv_node.output_anchors) != 1
            or len(conv_node.get_output_anchor(0).get_peer_input_anchor()) != 1
        ):
            return False
        ...
        skip_fusion_layers = (
            ...
            else self.config.get_skip_fusion_layers()
        )
        if conv_node.type != 'Conv' or conv_node.name in skip_fusion_layers:
            return False
    ...
    def do_pass(self, model, object_module, object_name, graph=None):
        ...
        if not self.is_fusionable_conv(conv_module, conv_name):
            return
        checked_model = torch.quantization.fuse_modules(
            model, [[conv_name, object_name]]
        )
        checked_module = ModuleHelper(checked_model).get_module(conv_name)
        if (
            torch.isinf(checked_module.weight).any()
            or torch.isnan(checked_module.weight).any()
            or torch.isinf(checked_module.bias).any()
...
def _delete_bn_from_graph(graph, conv_name, bn_name):
    ...
    # remove output links
    bn_peer_in_anchors = bn_node.get_output_anchor(0).get_peer_input_anchor()
    bn_peer_input_nodes = []
    ...
    for index, element in enumerate(bn_peer_input_nodes):
        graph.remove_edge(bn_node, 0, element, bn_peer_input_indexes[index])
