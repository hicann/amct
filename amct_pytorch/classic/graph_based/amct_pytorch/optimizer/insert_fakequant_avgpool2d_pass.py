...
from ...amct_pytorch.custom_op.fake_quant import FakeQuantizedAvgPool2d


class InsertFakequantAvgPool2dPass(BaseModuleFusionPass):
    """
    ...
    def match_pattern(self, module, name, graph=None):
        ...
        Return: True: matched
                False: mismatch
        """
        if type(module).__name__ == "AvgPool2d" and name in self.records:
            return True
        return False

    def do_pass(self, model, object_module, object_name, graph=None):
        """
        Function: insert fake quant AvgPool2d.
        Parameters:
