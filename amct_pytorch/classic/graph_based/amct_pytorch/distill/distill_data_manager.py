...
class DistillDataManager:
        ...
        with torch.no_grad():
            for module in modules:
                try:
                    ...
                    raise RuntimeError(
                        'module forward failed, exception is: {}'.format(e)
                    ) from e
    ...
    def get_input_data_by_inferring(self, model, group, sample):
        ...
        hook = module.register_forward_hook(
            self.cache_input_data_hooker(tensor, group[0])
        )
    ...
    def _dump_input_data_hooker(self, layer_name, epoch, step):
        ...
        def hook(module, inputs, outputs):
            ...
            LOGGER.logd(
                'dump layer {} input data success.'.format(layer_name),
                'DistillDataManager',
            )
    ...
    def _dump_group_output_data(self, model, groups, epoch, step):
        ...
        for group in groups:
            ...
            output_data = DistillDataManager.get_output_data_by_inferring(
                model, group, input_data
            )
            ...
            self.dump_files.append(file_name)
            LOGGER.logd(
                'dump layer {} output data success.'.format(group[0]),
                'DistillDataManager',
            )
