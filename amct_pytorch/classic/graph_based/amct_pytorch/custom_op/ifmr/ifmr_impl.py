...
def find_optimal_scale_offset(data, ifmr_param, max_value, min_value):
    ...
    max_candidates = torch.arange(
        start_ratio, end_ratio, step, dtype=torch.float32, device=device
    )
...
def find_max_and_min_value_with_percentile(data, max_percentile, min_percentile):
    ...
    max_value = max(torch.tensor(0.0, device=data.device), max_value)
    min_value = min(torch.tensor(0.0, device=data.device), min_value)

    return max_value, min_value


def ifmr_forward_pytorch(
    data,
    device,
    num_bits=8,
    with_offset=False,
    ...
    max_value, min_value = find_max_and_min_value_with_percentile(
        data, max_percentile, min_percentile
    )
    scale, offset = find_optimal_scale_offset(data, ifmr_param, max_value, min_value)

    clip_max = (
        torch.tensor(1.0, device=data.device)
        .mul_(BASE ** (num_bits - 1) - 1)
        .sub_(offset)
        .mul_(scale)
    )
    clip_min = (
        torch.tensor(1.0, device=data.device)
        .mul_(BASE ** (num_bits - 1))
        .add_(offset)
        .mul_(-scale)
    )
