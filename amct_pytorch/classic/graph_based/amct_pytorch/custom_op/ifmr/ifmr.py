...
class IFMR(nn.Module):  # pylint: disable=R0903
    ...
    def apply_ifmr(self, data, device, ifmr_param):
        ...
        scale, offset, clip_max, clip_min = ifmr_forward_pytorch(
            data,
            device.index,
    ...
    def _accm_data(self, inputs_list):
        ...
        if len(self.ifmr_data) == 0:
            ...
            for index, data in enumerate(inputs_list):
                check_quant_data(data, 'activation')
                self.ifmr_data[index] = torch.cat(
                    [self.ifmr_data[index], data.cpu().reshape([-1])]
                )
        else:
            raise RuntimeError(
                "Layer {} cout length({}) not equal to accumulated data cout length({})!".format(
                    self.layers_name, len(inputs_list), len(self.ifmr_data)
                ),
                'IFMR',
            )
        LOGGER.logd(
            "Doing layer {} data calibration: data already stored {}/{}".format(
