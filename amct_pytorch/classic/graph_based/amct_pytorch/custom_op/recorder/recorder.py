...
from ....amct_pytorch.common.utils.record_file_operator import read_weights_scale_offset
from ....amct_pytorch.common.utils.record_file_operator import (
...
class Recorder(nn.Module):  # pylint: disable=R0902
    ...
    def forward(self, layers_name, factors_type, quant_factors):
        """
    ...
    def _write_record_file(self):
        ...
        if self.__counts.get('act_cali_num') == self.__counts.get(LAYERS_NUM):
            with open(self.record_file, "w") as fid:
                fid.write(text_format.MessageToString(self.records, as_utf8=True))
            ...
            if self.__counts.get('kv_cache_quant_num') == self.__counts.get(
                'layers_num'
            ):
                with open(self.record_file, "w") as fid:
                    fid.write(text_format.MessageToString(self.records, as_utf8=True))
    ...
    def _add_acts_factors(self, layers_name, quant_factors):
        ...
        for layer_name in layers_name:
            ...
            record_activation_scale_offset(
                self.records, layer_name, scale_d, offset_d, num_bits, scale_h, offset_h
            )
            self.__counts['act_cali_num'] += 1
        LOGGER.logd(
            "Record layer '{}' data's quant factors!".format(layers_name), 'Recorder'
        )
    ...
    def _add_wts_factors(self, layers_name, quant_factors):
        ...
        for layer_name in layers_name:
            record_weights_scale_offset(
                self.records, layer_name, scale_w, offset_w, num_bits, scale_r, offset_r
            )
            self.__counts['wts_cali_num'] += 1
        LOGGER.logd(
            "Record layer '{}' weight's quant factors!".format(layers_name), 'Recorder'
        )
    ...
    def _add_dmq_balancer_factors(self, layers_name, quant_factors):
        ...
        LOGGER.logd(
            "Record layer '{}' tensor_balance_factor!".format(layers_name), 'Recorder'
        )
