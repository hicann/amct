...
from ....amct_pytorch.common.prune.selective_prune_helper_base import (
    SelectivePruneHelperBase,
)
...
class SelectivePruneFunction(Function):
    ...
    def forward(ctx, module_weight, n_out_of_m_type, prune_axis):
        ...
        return selective_mask_gen(
            module_weight.detach(), prune_axis, group_size, pruned_size
        )
...
def save_record_tofile(record_file, record):
    ...
    with os.fdopen(
        os.open(record_file, file_flags, file_mode), 'w', encoding='UTF-8', newline=''
    ) as fid:
...
def add_mask_to_record(layer_name, prune_mask):
    ...
    if len(pack_uint8) % 8 != 0:
        pack_uint8 = np.append(
            pack_uint8, np.zeros(8 - len(pack_uint8) % 8, dtype=np.uint8)
        )
    ...
    if (
        len(SingletonScaleOffsetRecord().record.record) != 0
        or len(SingletonScaleOffsetRecord().record.prune_record) != 0
    ):
...
def create_selective_prune_record(graph):
    ...
    for node in graph.nodes:
        ...
        SingletonScaleOffsetRecord().prune_record[node.name] = node.get_attr(
            'selective_prune_records'
        )[0]
