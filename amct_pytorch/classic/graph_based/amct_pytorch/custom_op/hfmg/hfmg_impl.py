...
def hfmg_merge_inter(
    data_bins, merged_data_bins, same_range, merged_data_min, merged_bin_width
):
    """
    ...
    if same_range:
        ...
        for data_bin in data_bins:
            ...
            if idx_low == idx_high:
                ...
                scale_of_bin = (merged_data_bins[idx_low].higher_bound - lower) / (
                    upper - lower
                )
...
def hfmg_calculate_loss(data_bins, dst_num_bins, search_range):
    ...
    data_bin_count = torch.tensor(
        [bin_data.count for bin_data in data_bins], device=device
    )
    ...
    src_bin_begins = (
        bins_arange.reshape(-1, 1).repeat(1, range_len) - bin_index_lefts
    ) * bin_width
    ...
    half_dst_bin_widths = (
        (dst_bin_widths / BASE).reshape(-1, range_len).repeat(num_bins, 1)
    )
    ...
    l2_loss = get_norm(
        delta_begins * same_bin_flag,
        delta_ends * same_bin_flag,
        densities * same_bin_flag,
    ).sum(dim=0)

    # the left dst bin and src bin overlapping parts
    l2_loss += get_norm(
        delta_begins * diff_bin_flag,
        half_dst_bin_widths * diff_bin_flag,
        densities * diff_bin_flag,
    ).sum(dim=0)
    ...
    delta_ends = (
        src_bin_ends - dst_bin_end_idxs * dst_bin_widths - half_dst_bin_widths
    ) * diff_bin_flag
    l2_loss += get_norm(
        -half_dst_bin_widths * diff_bin_flag, delta_ends, densities * diff_bin_flag
    ).sum(dim=0)

    l2_loss = torch.where(
        dst_bin_widths < FLT_EPSILON, torch.tensor(0.0, device=device), l2_loss
    )
...
def hfmg_compute(data_bins, num_bits, with_offset):
    ...
    scale, offset = calculate_scale_offset(
        clip_max, clip_min, with_offset, "INT" + str(num_bits)
    )
...
def hfmg_arq_pytorch(min_tensor, max_tensor, num_bits, with_offset):
    ...
    """
    scale, offset = calculate_scale_offset(
        max_tensor, min_tensor, with_offset, "INT" + str(num_bits)
    )
...
def hfmg_merge_pytorch(hist, hist_range, new_hist, new_hist_range, nbins=4096):
    ...
    same_range = math.isclose(
        new_hist_range[0], hist_range[0], abs_tol=FLT_EPSILON
    ) and math.isclose(new_hist_range[-1], hist_range[-1], abs_tol=FLT_EPSILON)

    hfmg_merge_inter(
        data_bins, merged_data_bins, same_range, merged_min, merged_bin_width
    )

    merged_hist = torch.tensor(
        [bin.count for bin in merged_data_bins], dtype=torch.int32, device=hist.device
    )
...
def hfmg_forward_pytorch(hist, hist_range, num_bits=8, with_offset=False, nbins=4096):
    ...
    clip_max_out = (
        ...
        .mul_(BASENUM ** (num_bits - 1) - 1)
        .sub_(offset)
        .mul_(scale)
    )
    clip_min_out = (
        torch.tensor(1.0, device=device)
        .mul_(BASENUM ** (num_bits - 1))
        .add_(offset)
        .mul_(-scale)
    )
