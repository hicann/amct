...
class HFMG(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if not do_calibration:
            ...
            scale, offset = hfmg_arq_pytorch(
                input_min,
                input_max,
                self.param.get('num_bits'),
                self.param.get('with_offset'),
            )

            return QuantInfo._make(
                [self.calibrated_flag, [scale], [offset], clip_max, clip_min]
            )
        ...
        for min_val, max_val, histogram in self.hfmg_data:
            ...
            scale, offset, clip_max, clip_min = hfmg_forward_pytorch(
                histogram,
                min_max,
        ...
        return QuantInfo._make(
            [self.calibrated_flag, scale_list, offset_list, clip_max, clip_min]
        )
        ...
        if len(self.hfmg_data) == 0:
            for index, data in enumerate(inputs_list):
                ...
                histogram = torch.histc(
                    data, self.nbins, min=histc_min, max=histc_max
                ).contiguous()
            ...
            for index, data in enumerate(inputs_list):
                ...
                combined_min_max = torch.cat(
                    [combined_min.reshape(-1), combined_max.reshape(-1)]
                )
                ...
                min_max = torch.cat([min_val.reshape(-1), max_val.reshape(-1)])
                merged_hist = hfmg_merge_pytorch(
                    self.hfmg_data[index][2],
                    min_max,
                    new_hist,
                    combined_min_max,
                    self.nbins,
            ...
            raise RuntimeError(
                "Layer {} cout length({}) not equal to accumulated data cout length({})!".format(
                    self.layers_name, len(inputs_list), len(self.hfmg_data)
                ),
                'HFMG',
            )
