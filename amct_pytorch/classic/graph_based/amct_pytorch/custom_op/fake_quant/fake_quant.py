...
class FakeQuant(nn.Module):  # pylint: disable=R0903, R0902
    ...
    def __init__(
        ...
        self.clamp_min = torch.tensor(-(2 ** (num_bits - 1)), requires_grad=False)
        self.clamp_max = torch.tensor(2 ** (num_bits - 1) - 1, requires_grad=False)
    ...
    def forward(self, inputs):  # pylint: disable=W0221
        ...
        temp_data = torch.add(
            torch.round(torch.mul(inputs, self.scale_d.to(device))),
            self.offset_d.to(device),
        )
        clamped_data = torch.clamp(
            temp_data, self.clamp_min.to(device), self.clamp_max.to(device)
        )
    ...
    def extra_repr(self):
        ...
        extra_s = ('scale_d={}, offset_d={}, clamp_min={}, clamp_max={}').format(
