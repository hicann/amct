...
class DMQBalancerFunction(Function):
    ...
    def forward(ctx, inputs, weights, migration_strength):
        ...
        tensor_balance_factor = dmq_balancer_forward(
            inputs, weights, migration_strength
        )
...
def dmq_balancer_forward(inputs, weights, migration_strength):
    ...
    if migration_strength < 0.2 or migration_strength > 0.8:
        raise ValueError(
            f'migration_strength:{migration_strength} not support, should be in [0.2, 0.8]!'
        )

    if (
        inputs.abs().max() < torch.finfo(inputs.dtype).eps
        or weights.abs().max() < torch.finfo(weights.dtype).eps
    ):
    ...
    tensor_balance_factor = torch.where(
        wts_zero_mask,
        torch.tensor(0, dtype=act_max.dtype, device=act_max.device),
        (act_max**migration_strength) / (wts_max ** (1 - migration_strength)),
    )

    dmq_zero_mask = tensor_balance_factor < torch.finfo(act_max.dtype).eps
    act_outlier = torch.where(
        dmq_zero_mask,
        torch.tensor(0, dtype=act_max.dtype, device=act_max.device),
        (act_max / tensor_balance_factor),
    ).max()
    wts_outiler = torch.where(
        dmq_zero_mask,
        torch.tensor(0, dtype=act_max.dtype, device=act_max.device),
        (wts_max * tensor_balance_factor),
    ).max()

    tensor_balance_factor[act_zero_mask & wts_zero_mask] = torch.tensor(
        1.0, dtype=tensor_balance_factor.dtype, device=tensor_balance_factor.device
    )
    tensor_balance_factor[act_zero_mask & (~wts_zero_mask)] = (
        wts_outiler / wts_max[act_zero_mask & (~wts_zero_mask)]
    )
    tensor_balance_factor[(~act_zero_mask) & wts_zero_mask] = (
        act_max[(~act_zero_mask) & wts_zero_mask] / act_outlier
    )
