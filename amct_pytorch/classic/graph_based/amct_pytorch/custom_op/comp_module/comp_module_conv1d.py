...
class CompModuleConv1d(CompModuleBase):
    """
    Function: Quantization module class after Conv1d encapsulation.
    APIs: __init__, forward
    """

    def __init__(self, *args, **kwargs):
        super(CompModuleConv1d, self).__init__(*args, **kwargs)
    ...
    def forward(self, inputs):
        ...
        """
        compressed_inputs, compressed_weights = super(CompModuleConv1d, self).forward(
            inputs
        )

        # amct_pytorch only support Conv1d with padding_mode 'zeros'
        padding_mode = self.replaced_module.padding_mode
        if 'quant' in self.comp_algs and padding_mode != 'zeros':
            raise RuntimeError(
                'Do not support Conv1d with padding mode {}!'.format(padding_mode)
            )
