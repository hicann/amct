...
def selective_mask_gen(tensor, prune_axis, group_size, pruned_size):
    ...
    if prune_axis >= tensor.dim():
        raise RuntimeError("selective input tensor dim is less than prune axis.")
    ...
    group_shape, padding_shape, ungroup_shape = cal_shape(
        ori_tensor_shape, prune_axis, group_num, group_size, padding_size
    )
    ...
    sort_values, sort_indices = tensor_padding_group.topk(
        pruned_size, prune_axis + 1, largest=False
    )
    # gen mask
    mask = torch.ones_like(tensor_padding_group).scatter_(
        dim=prune_axis + 1, index=sort_indices, value=0
    )
    result = torch.split(
        mask.view(ungroup_shape), tensor.size(prune_axis), dim=prune_axis
    )
    return result[0]
