...
class PuneIndexHelper:
    ...
    def do_bcp(input_tensors, prune_axises, bcp_param):
        ...
        cout_mask = bcp(
            input_tensors,
            prune_axises,
            bcp_param.get('prune_ratio'),
            bcp_param.get('prune_group'),
            bcp_param.get('ascend_optimized'),
        )
        return cout_mask
    ...
    def prepare_param(self):
        ...
        for producer in self.record.producer:
            ...
            if self.global_prune_ratio is None:
                ...
                self.global_prune_ratio = min(
                    self.global_prune_ratio, layer_prune_config.get(PRUNE_RATIO)
                )
            if layer_prune_config.get("ascend_optimized") and layer_prune_config.get(
                "ascend_optimized"
            ):
            ...
            if cout_axis == 0:
                ...
                raise RuntimeError(
                    'unexpected weight cout axis is {}'.format(cout_axis)
                )
    ...
    def add_to_record(self, prune_cout):
        ...
        if not prune_cout:
            LOGGER.logd(
                'no index to prune {}.'.format(
                    [producer.name for producer in self.record.producer]
                )
            )
            return
        for producer in self.record.producer:
