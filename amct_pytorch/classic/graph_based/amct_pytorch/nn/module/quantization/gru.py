...
class GRUQAT(nn.GRU, RnnQatBase):
    ...
    def forward(self, inputs, hx=None):
        ...
        if not isinstance(hx, torch.Tensor):
            raise RuntimeError(
                'GRUQAT hx must be Tensor, but your input is {}'.format(type(hx))
            )
        if len(hx.shape) != RNN_INPUT_DIM:
            raise RuntimeError(
                "GRU quantize only support input hx dim 3,"
...
class GRUMiddleWare(torch.autograd.Function):
    ...
    def forward(
        ...
        mod.weight_ih_l0 = Parameter(
            RnnQatBase.reorganize_tensor(w, 'GRU', back=True).to(device=x.device)
        )
        mod.weight_hh_l0 = Parameter(
            RnnQatBase.reorganize_tensor(r, 'GRU', back=True).to(device=x.device)
        )
