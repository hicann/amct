...
class ConvTranspose1dQAT(nn.ConvTranspose1d, QATBase):
    ...
    def __init__(self,
                 ...
                 dtype=None,
                 config=None):
        nn.ConvTranspose1d.__init__(self, in_channels, out_channels, kernel_size,
                                    stride=stride, padding=padding, dilation=dilation,
                                    groups=groups, bias=bias, padding_mode=padding_mode)
        QATBase.__init__(self, 'ConvTranspose1d',
                         device=device, config=config)

    def check_quantifiable(self):
        if self.retrain_enable and self.padding_mode != 'zeros':
            raise ValueError(f'Do not support ConvTranspose1d with padding mode {self.padding_mode}')
