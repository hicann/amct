...
class ConvTranspose1dQAT(nn.ConvTranspose1d, QATBase):
    ...
    def __init__(
        ...
        QATBase.__init__(self, 'ConvTranspose1d', device=device, config=config)

    def check_quantifiable(self):
        if self.retrain_enable and self.padding_mode != 'zeros':
            raise ValueError(
                f'Do not support ConvTranspose1d with padding mode {self.padding_mode}'
            )
    ...
    def forward(self, inputs, output_size=None):
        ...
        if version_higher_than(torch.__version__, '1.12.0'):
            ...
            output_padding = self._output_padding(
                inputs, output_size, self.stride, self.padding, self.kernel_size
            )
