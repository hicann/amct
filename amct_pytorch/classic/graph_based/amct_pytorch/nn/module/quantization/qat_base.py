...
BIAS = 'bias'
SUPPORT_RETRAIN_ACT_WTS_TYPE_MAP = {INT8: INT8, INT16: INT8}

ULQRetrainParams = namedtuple(
    'ULQRetrainParams',
    ['scale', 'offset', 'clip_max', 'clip_min', 'clip_max_pre', 'clip_min_pre'],
)
WtsRetrainParams = namedtuple(
    'WtsRetrainParam', ['scale', 'offset', 'offset_deploy', 'wts_group_num']
)
...
class QATBase(metaclass=ABCMeta):
    ...
    def __init__(self, layer_type, device, config=None):
        ...
        if not isinstance(config, dict):
            raise TypeError(
                f'config should be none or dict, but your input is {type(config)}'
            )
        ...
        self.retrain_weight_config = config.get('retrain_weight_config', dict())
    ...
    def do_ifmr(inputs, ifmr_module, distribute_config):
        is_init, scale_list, offset_list, clip_max, clip_min = ifmr_module.forward(
            inputs
        )
        scale = scale_list[0]
    ...
    @staticmethod
    def do_ulq_retrain(
        inputs, ulq_retrain_params, retrain_data_config, distribute_config, training
    ):
        """
        ...
        outputs, scale, offset, clip_max, clip_min = UlqRetrainFuncQAT.apply(
            inputs,
            ulq_retrain_params.clip_max,
            ulq_retrain_params.clip_min,
            ulq_retrain_params.clip_max_pre,
            ulq_retrain_params.clip_min_pre,
            act_param,
            None,
            need_sync,
            distribute_config.get('process_group'),
    ...
    def _register_qat_params(module):
        ...
        module.register_parameter(
            ...
            Parameter(
                tensor(
                    module.retrain_data_config.get(CLIP_MAX, 1.0),
        ...
        module.register_parameter(
            ...
            Parameter(
                tensor(
                    module.retrain_data_config.get(CLIP_MIN, -1.0),
                    device=module.quant_device,
                ),
                requires_grad=True,
            ),
        ...
        module.register_buffer(
            'acts_clip_max_pre', tensor(np.nan, device=module.quant_device)
        )
        module.register_buffer(
            'acts_clip_min_pre', tensor(np.nan, device=module.quant_device)
        )
        module.register_buffer(
            'acts_scale', tensor([np.nan], device=module.quant_device)
        )
        module.register_buffer(
            'acts_offset_deploy',
        ...
        module.register_parameter(
            'wts_scales',
            Parameter(
                tensor([np.nan] * num_channel, device=module.quant_device),
                requires_grad=False,
            ),
        )
        module.register_parameter(
        ...
        module.register_buffer(
            'wts_offsets_deploy',
            tensor([0] * num_channel, dtype=torch.int8, device=module.quant_device),
        )
        module.register_buffer(
            's_rec_flag',
            tensor(
                module.retrain_weight_config.get('s_rec_flag', False),
                device=module.quant_device,
            ),
        )
    ...
    def _get_ori_op_params(cls, mod):
        ...
        if err_params:
            raise RuntimeError(
                f'The following parameters are not found in the torch operator. {err_params}'
    ...
    def forward_qat(self, inputs):
        if inputs.dtype is not torch.float32:
            raise ValueError(
                'Only support dtype torch.float32, but your input dtype is {}'.format(
                    inputs.dtype
                )
            )
    ...
    def acts_quant_init(self, inputs):
        ...
        with torch.no_grad():
            ...
            copy_tensor(
                self.acts_offset_deploy, ulq_retrain_params.offset.to(self.offset_dtype)
            )
    ...
    def acts_quant(self, inputs):
        ...
        """
        ulq_retrain_params = ULQRetrainParams(
            self.acts_scale,
        ...
        with torch.no_grad():
            ...
            copy_tensor(
                self.acts_offset_deploy,
                ulq_retrain_params_update.offset.to(self.offset_dtype),
            )
    ...
    def wts_quant(self):
        ...
        weights_retrain_algo = self.retrain_weight_config.get(
            'weights_retrain_algo', 'arq_retrain'
        )
        wts_quant_params = WtsRetrainParams(
            self.wts_scales,
            self.wts_offsets,
            self.wts_offsets_deploy,
            self.wts_group_num,
        )
        quantized_weight, scales, offsets = wts_quant_dict.get(weights_retrain_algo)(
            self.weight, wts_quant_params
        )
    ...
    def wts_quant_arq(self, wts, wts_retrain_params):
        ...
        quantized_weight, scale, offset = ArqRetrainFuncQAT.apply(
            wts,
    ...
    def wts_quant_ulq(self, wts, wts_retrain_params):
        ...
        quantized_weight, scale, offset = UlqScaleRetrainFuncQAT.apply(
            wts,
            wts_retrain_params.scale,
            wts_retrain_params.offset,
            wts_param,
    ...
    def _check_qat_config(self):
        ...
        if self.retrain_data_config.get(DST_TYPE, INT8) not in [INT8, INT16]:
            raise ValueError(
                "dst_type for activation should be in range ['INT8', 'INT16'], "
        ...
        if not isinstance(batch_num, int) or batch_num <= 0:
            raise ValueError(
                "batch_num should be a int bigger than 0, but your input is {}".format(
                    batch_num
                )
            )
        ...
        if not isinstance(self.retrain_data_config.get(FIXED_MIN, False), bool):
            raise ValueError(
                "fixed_min should be a bool value, but your input is {}".format(
        ...
        if not isinstance(clip_min, float) or clip_min >= 0.0:
            raise ValueError(
                'clip_min should be a float smaller than zero, but your input is {}.'.format(
                    clip_min
                )
            )
        if not isinstance(clip_max, float) or clip_max <= 0.0:
            raise ValueError(
                'clip_max should a float be bigger than zero, but your input is {}.'.format(
                    clip_max
                )
            )
        ...
        if self.retrain_weight_config.get('weight_retrain_algo', 'arq_retrain') not in [
            ...
            raise ValueError(
                "weight_retrain_algo should be in range ['arq_retrain', 'ulq_retrain'], "
                "but your input is {}".format(self.retrain_weight_config.get(DST_TYPE))
            )

        if not isinstance(self.retrain_weight_config.get('channel_wise', True), bool):
            raise ValueError(
                "channel_wise should be a bool value, but your input is {}".format(
