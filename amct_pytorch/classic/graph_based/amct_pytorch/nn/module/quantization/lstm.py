...
class LSTMQAT(nn.LSTM, RnnQatBase):
    ...
    def from_float(cls, mod, config=None):
        ...
        if not isinstance(mod, cls._float_module):
            raise RuntimeError(
                '{}.from_float can only works for {}'.format(
        ...
        LOGGER.logi(
            'Convert {} to QAT op successfully.'.format(cls._float_module.__name__)
        )
        return qat_op
    ...
    def check_quantifiable(self):
        ...
        if hasattr(self, 'proj_size') and self.proj_size != 0:
            raise RuntimeError(
                'Do not support LSTM with proj_size {}'.format(self.proj_size)
            )
        return True
    ...
    def forward(self, inputs, hx=None):
        ...
        if (
            not isinstance(hx, Iterable)
            or not isinstance(hx[0], torch.Tensor)
            or not isinstance(hx[1], torch.Tensor)
        ):
            raise RuntimeError(
                'LSTMQAT hx must be (Tensor, Tensor), but your input is {}'.format(
                    type(hx)
                )
            )
        if len(hx[0].shape) != RNN_INPUT_DIM:
            raise RuntimeError(
                "LSTM quantize only support input hx dim 3,"
        ...
        with torch.enable_grad():
            y, y_h, y_c = LSTMMiddleWare.apply(
                quantized_acts,
                quantized_h_x,
                hx[1],
                quantized_wts,
...
class LSTMMiddleWare(torch.autograd.Function):
    ...
    def symbolic(
        ...
        shape = g.op(
            'Constant',
            value_t=torch.tensor([1, batch_size, hidden_size], dtype=torch.int64),
        )
    ...
    def forward(
        ...
        mod = torch.nn.LSTM(
            ...
            hidden_size,
            batch_first=batch_first,
            bias=bias_ih_l0 is not None,
        )
        mod.weight_ih_l0 = Parameter(
            RnnQatBase.reorganize_tensor(w, 'LSTM', back=True).to(device=x.device)
        )
        mod.weight_hh_l0 = Parameter(
            RnnQatBase.reorganize_tensor(r, 'LSTM', back=True).to(device=x.device)
        )
