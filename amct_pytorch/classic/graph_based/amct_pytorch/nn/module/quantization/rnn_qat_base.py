...
from .....amct_pytorch.nn.module.quantization.qat_base import (
    QATBase,
    ULQRetrainParams,
    WtsRetrainParams,
)
...
class RnnQatBase(QATBase):
    def __init__(self, layer_type, device, config):
        ...
        if not config.get('from_float'):
            self.weight = Parameter(
                RnnQatBase.reorganize_tensor(self.weight_ih_l0, self.layer_type).to(
                    device=device
                )
            )
            self.recurrence_weight = Parameter(
                RnnQatBase.reorganize_tensor(self.weight_hh_l0, self.layer_type).to(
                    device=device
                )
            )
            if hasattr(self, 'bias_hh_l0'):
                self.bias_value = torch.cat(
                    [
                        RnnQatBase.reorganize_tensor(
                            self.bias_ih_l0.detach(), self.layer_type
                        ),
                        RnnQatBase.reorganize_tensor(
                            self.bias_hh_l0.detach(), self.layer_type
                        ),
                    ]
                ).reshape(1, -1)
            else:
    ...
    def set_rnn_op_trained_params(mod, qat_op, layer_type):
        ...
        weight = torch.nn.Parameter(
            RnnQatBase.reorganize_tensor(mod.weight_ih_l0, layer_type).to(device=device)
        )
        ...
        if mod.bias:
            ...
            setattr(
                ...
                torch.cat(
                    [
                        RnnQatBase.reorganize_tensor(bias.detach(), layer_type),
                        RnnQatBase.reorganize_tensor(
                            recurrence_bias.detach(), layer_type
                        ),
                    ]
                ).reshape(1, -1),
            )
    ...
    def check_quantifiable(self):
        ...
        if self.num_layers != 1:
            raise RuntimeError(
                'Do not support {} with num_layers {}'.format(
                    self.layer_type, self.num_layers
                )
            )
        if self.bidirectional:
            raise RuntimeError(
                'Do not support {} with bidirectional {}'.format(
                    self.layer_type, self.bidirectional
                )
            )
        if self.dropout != 0:
            raise RuntimeError(
                'Do not support {} with dropout {}'.format(
                    self.layer_type, self.dropout
                )
            )
        return True
    ...
    def check_input_data(self, inputs, hx):
        ...
        if not isinstance(inputs, torch.Tensor):
            raise RuntimeError(
                '{}QAT only support tensor input, but your input type is {}'.format(
        ...
        if not isinstance(inputs, torch.Tensor):
            raise RuntimeError(
                '{}QAT op only support tensor input, but your input type is {}'.format(
                    self.layer_type, type(input)
                )
            )
        ...
        if len(inputs.shape) != RNN_INPUT_DIM:
            raise RuntimeError(
                "{} quantize only support input dim 3, but your input dim is {}".format(
                    self.layer_type, len(inputs.shape)
                )
            )
    ...
    def for_loop_rnncell_forward(self, inputs, hx, module_type, params):
        ...
        for i in range(0, seq_len):
            h_next = quant_dequant_tensor(
                h_next, self.h_scale, self.h_offset_deploy, self.act_num_bits
            )
            xi = inputs[:, i, :]
            if module_type == "GRU":
        ...
        if self.retrain_enable:
            ...
            if self.h_do_init:
                ...
                if seq_len > 1:
                    ...
                    initial_h, _ = self.for_loop_rnncell_forward(
                        inputs, hx, self.layer_type, params
                    )
            ...
            quantized_h, quantized_recurrence_weights = (
                initial_h,
                self.recurrence_weight,
            )
        ret = (
    ...
    def _h_quant_init(self, initial_h):
        ...
        with torch.no_grad():
            ...
            copy_tensor(
                self.h_offset_deploy, ulq_retrain_params.offset.to(self.offset_dtype)
            )
    ...
    def _recurrence_wts_quant(self):
        ...
        rec_wts_param = WtsRetrainParams(
            self.recurrence_wts_scales,
            self.recurrence_wts_offsets,
            self.recurrence_wts_offsets_deploy,
            self.wts_group_num,
        )
        quantized_recurrence_weight, scales, offsets = wts_quant_dict.get(
            weights_retrain_algo
        )(self.recurrence_weight, rec_wts_param)
    ...
    def _register_rnn_qat_params(self):
        ...
        self.register_parameter(
            ...
            Parameter(
                tensor(
                    self.retrain_data_config.get(CLIP_MAX, 1.0),
        ...
        self.register_parameter(
            ...
            Parameter(
                tensor(
                    self.retrain_data_config.get(CLIP_MIN, -1.0),
                    device=self.quant_device,
                ),
                requires_grad=True,
            ),
        )
        self.register_buffer('h_clip_max_pre', tensor(np.nan, device=self.quant_device))
        self.register_buffer('h_clip_min_pre', tensor(np.nan, device=self.quant_device))
        self.register_buffer('h_scale', tensor([np.nan], device=self.quant_device))
        ...
        self.register_parameter(
            'recurrence_wts_scales',
            Parameter(tensor([np.nan] * num_channel, device=self.quant_device)),
        )
        self.register_parameter(
            'recurrence_wts_offsets',
            Parameter(tensor([np.nan] * num_channel, device=self.quant_device)),
        )
        self.register_buffer(
            'recurrence_wts_offsets_deploy',
            tensor([0] * num_channel, dtype=torch.int8, device=self.quant_device),
        )
