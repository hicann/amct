...
class MatMulQAT(nn.Module):
    @check_params(device=(str, type(None)), config=(dict, type(None)))
    def __init__(self, device=None, config=None):
        ...
        act_num_bits = NUM_BITS_MAP.get(self.retrain_data_config.get('dst_type', INT8))
        ...
        self.input_init_module = IFMR(
            layers_name='input_ifmr',
            num_bits=act_num_bits,
            batch_num=self.retrain_data_config.get(BATCH_NUM, 1),
        )
        self.other_init_module = IFMR(
            layers_name='other_ifmr',
            num_bits=act_num_bits,
            batch_num=self.retrain_data_config.get(BATCH_NUM, 1),
        )
    ...
    def check_input(input_data):
        ...
        if len(input_data.shape) < 2 or len(input_data.shape) > 6:
            raise RuntimeError(
                'MatMul dual input quantize only support input dim 2-6,'
    ...
    def acts_quant(self, data, quant_params):
        ...
        ulq_retrain_params = ULQRetrainParams(
            quant_params.get('scale'),
            quant_params.get('offset_deploy'),
            quant_params.get('clip_max'),
            quant_params.get('clip_min'),
            quant_params.get('clip_max_pre'),
            quant_params.get('clip_min_pre'),
        )
        ...
        with torch.no_grad():
            ...
            copy_tensor(
                quant_params.get('offset_deploy'),
                ulq_retrain_params_update.offset.to(torch.int8),
            )
            copy_tensor(
                quant_params.get('clip_max'), ulq_retrain_params_update.clip_max
            )
            copy_tensor(
                quant_params.get('clip_min'), ulq_retrain_params_update.clip_min
            )
            copy_tensor(
                quant_params.get('clip_max_pre'), ulq_retrain_params_update.clip_max
            )
            copy_tensor(
                quant_params.get('clip_min_pre'), ulq_retrain_params_update.clip_min
            )
        return outputs
    ...
    def acts_quant_init(self, data, quant_params, ifmr_module):
        ...
        with torch.no_grad():
            ...
            copy_tensor(
                quant_params.get('offset_deploy'),
                ulq_retrain_params.offset.to(torch.int8),
            )
    ...
    def _check_qat_config(self):
        ...
        """
        if self.retrain_data_config.get(DST_TYPE, INT8) not in [INT8]:
            raise ValueError(
                "dst_type for activation should be in range ['INT8'], "
                "but your input is {}".format(
    ...
    def _register_qat_params(self):
        ...
        self.register_parameter(
            'input_clip_max',
            Parameter(
                tensor(self.retrain_data_config.get(CLIP_MAX, 1.0), device=self.device),
                requires_grad=True,
            ),
        )
        self.register_parameter(
            ...
            Parameter(
                tensor(
                    self.retrain_data_config.get(CLIP_MIN, -1.0), device=self.device
                ),
                requires_grad=True,
            ),
        )
        ...
        self.register_buffer(
            'input_offset_deploy', tensor([0], dtype=torch.int8, device=self.device)
        )
        ...
        self.register_buffer(
            'other_offset_deploy', tensor([0], dtype=torch.int8, device=self.device)
        )
