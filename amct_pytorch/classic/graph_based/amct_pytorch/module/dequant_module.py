...
def add_fake_dequant(graph, layer_name, dqscale):
    ...
    # add scale
    dqscale_node = graph.add_node(_construct_fake_dequant_dqscale(layer_name, dqscale))
    # add mul
    ...
    if dtype == 'float16':
        cast_node = graph.add_node(
            construct_fake_quant_dequant_cast_op(layer_name, "float16")
        )
...
def _construct_fake_dequant_clip(layer_name):
    ...
    node_proto.input.extend(
        [  # pylint: disable=E1101
            '.'.join([layer_name, 'div.output0', OUTPUT_0]),
