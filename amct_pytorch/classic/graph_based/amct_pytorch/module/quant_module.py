...
def add_fakequant(graph, layer_name, scale, offset, quant_bit):
    ...
    offset_node = graph.add_node(_construct_fakequant_offset(layer_name, offset))
    ...
    if dtype == 'float16':
        cast_node = graph.add_node(
            construct_fake_quant_dequant_cast_op(layer_name, "float32")
        )
...
def construct_fake_quant_dequant_cast_op(layer_name, to_dtype):
    ...
    node_proto.input.extend(['.'.join([layer_name, CAST, 'input0'])])
    node_proto.output.extend(['.'.join([layer_name, CAST, OUTPUT_0])])
