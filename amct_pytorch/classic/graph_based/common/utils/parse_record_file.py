...
from ..config.field import ACT_SUPPORT_NUM_BITS
...
RecordFileParsedResult = collections.namedtuple(
    'RecordFileParsedResult', ['layers_params', 'skip_fusion_layers']
)
...
class RecordFileParserBase:
    ...
    def is_records_empty(self):
        ...
        if self.enable_quant and list(self.records.record) == []:
            return True
        if self.enable_prune and list(self.records.prune_record) == []:
            return True
        return False
    ...
    def parse(self, enable_shift_n=True):  # pylint: disable=R0912
        ...
        if not self.enable_quant and not self.enable_prune:
            raise ValueError("Can not parse record file.because enable are both False.")
        ...
        for record in self.records.prune_record:
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    % (node_name, self.model_name, self.model_name, self.record_file)
                ) from e
    ...
    def parse_quant(self, enable_shift_n):
        ...
        skip_fusion_layers = []
        quantizable_layers = self.config[GRAPH_QUERIER].get_support_quant_layers(
            self.graph
        )
        if hasattr(
            self.config[GRAPH_QUERIER], 'get_support_qat_layer2type'
        ) and not hasattr(self.graph, 'get_operations'):
            quantizable_layers += list(
                self.config[GRAPH_QUERIER].get_support_qat_layer2type(self.graph).keys()
            )
            ...
            if node.name in quantizable_layers:
                ...
                if layer_params:
                    ...
                    if self.config.get(GRAPH_CHECKER) is not None:
                        self.config[GRAPH_CHECKER].check_quantize_placeholder(
                ...
                if tensor_balance_factor:
                    ...
                    layers_params[record.key]['tensor_balance_factor'] = (
                ...
                if self.config.get(GRAPH_CHECKER) is not None:
                    ...
                    self.config[GRAPH_CHECKER].check_tensor_quant(
                        self.graph,
                        [{'layer_name': layer_name, 'input_index': int(input_index)}],
                    )
    ...
    def find_layer_in_graph(self, record):
        ...
        try:
            ...
            raise RuntimeError(
                "find %s failure in %s. Please match %s and %s."
                % (record.key, self.model_name, self.model_name, self.record_file)
            ) from e
...
class RecordManager:
    ...
    def is_in_range(value_array, min_val, max_val, included=True):
        ...
        if included:
            is_in_range = (value_array >= min_val) & (value_array <= max_val)
        else:
            is_in_range = (value_array > min_val) & (value_array < max_val)
    ...
    def check_rnn_layer_params_range(layer_params, layer_name):
        ...
        if is_invalid(layer_params.get(H_OFFSET)) or not RecordManager.is_in_range(
            layer_params.get(H_OFFSET), data_offset_range[0], data_offset_range[1]
        ):
            raise ValueError(
                "offset_h's value should be in %s in layer %s."
                % (data_offset_range, layer_name)
            )
        # offset_r
        if is_invalid(
            layer_params.get(RECURRENCE_WEIGHT_OFFSET)
        ) or not RecordManager.is_in_range(
            layer_params.get(RECURRENCE_WEIGHT_OFFSET),
            WEIGHT_OFFSET_RANGE[0],
            WEIGHT_OFFSET_RANGE[1],
        ):
            raise ValueError("offset_r's value should be 0 in layer %s." % (layer_name))
        # scale_d
        if is_invalid(layer_params.get('h_scale')) or not RecordManager.is_in_range(
            layer_params.get('h_scale'), SCALE_RANGE[0], SCALE_RANGE[1], included=False
        ):
            raise ValueError(
                "scale_h's value shoule be in positivate range "
                "of float32 in layer %s." % (layer_name)
            )
        # scale_r
        if is_invalid(
            layer_params.get('recurrence_weight_scale')
        ) or not RecordManager.is_in_range(
            layer_params.get('recurrence_weight_scale'),
            SCALE_RANGE[0],
            SCALE_RANGE[1],
            ...
            raise ValueError(
                "scale_r's value shoule be in positivate range "
                "of float32 in layer %s" % (layer_name)
            )
    ...
    def check_cluster_center(cluster_center_array):
        ...
        for value in cluster_center_array:
            if value < CLUSTER_CENTER_RANGE[0] or value > CLUSTER_CENTER_RANGE[1]:
                raise ValueError(
                    "Cluster center should between [-128, 127], but is {}".format(value)
                )
    ...
    def check_layer_params_range(cls, layer_params, layer_name):
        ...
        # offset_d
        data_offset_range = RecordManager.get_range(layer_params['act_type'])
        if is_invalid(layer_params.get(DATA_OFFSET)) or not RecordManager.is_in_range(
            layer_params.get(DATA_OFFSET), data_offset_range[0], data_offset_range[1]
        ):
            raise ValueError(
                "offset_d's value should be in %s in layer %s."
                % (data_offset_range, layer_name)
            )
        # offset_w
        if is_invalid(layer_params.get(WEIGHT_OFFSET)) or not RecordManager.is_in_range(
            layer_params.get(WEIGHT_OFFSET),
            WEIGHT_OFFSET_RANGE[0],
            WEIGHT_OFFSET_RANGE[1],
        ):
            raise ValueError("offset_w's value should be 0 in layer %s." % (layer_name))
        # scale_d
        if is_invalid(layer_params.get(DATA_SCALE)) or not RecordManager.is_in_range(
            layer_params.get(DATA_SCALE), SCALE_RANGE[0], SCALE_RANGE[1], included=False
        ):
            raise ValueError(
                "scale_d's value shoule be in positivate range "
                "of float32 in layer %s." % (layer_name)
            )
        # scale_w
        if is_invalid(layer_params.get(WEIGHT_SCALE)) or not RecordManager.is_in_range(
            layer_params.get(WEIGHT_SCALE),
            SCALE_RANGE[0],
            SCALE_RANGE[1],
            included=False,
        ):
            raise ValueError(
                "scale_w's value shoule be in positivate range "
                "of float32 in layer %s" % (layer_name)
            )
        ...
        if is_invalid(layer_params.get(SHIFT_N)) or not _is_shift_n_in_range(
            layer_params.get(SHIFT_N)
        ):
            raise ValueError(
                "shift_bit's value shoule be in %s or all be 0 "
    ...
    def get_pruned_quant_factor(
        self, scale_w_array, offset_w_array, shift_n_array, enable_shift_n
    ):
        """
        ...
        if prune_channel and channel_wise:
            ...
            if factor_length != mask_length:
                raise RuntimeError(
                    'The quant factor length {} does not match '
    ...
    def parse_quant_value_rnn(self, layer_params):
        ...
        scale_r_array, offset_r_array, _ = self.adjust_scale_offset_shiftn(
            scale_r_array, offset_r_array, np.array([])
        )
        ...
        if layer_params['act_type'] == 'INT16':
            layer_params[H_OFFSET] = layer_params[H_OFFSET].astype(np.int16)
        else:
            layer_params[H_OFFSET] = layer_params[H_OFFSET].astype(np.int8)
        layer_params[RECURRENCE_WEIGHT_OFFSET] = layer_params[
    ...
    def parse_quant_value(self, enable_shift_n=True, enable_wts_quant=True):
        ...
        # read scale_w, offset_w and shift_bit
        scale_w_array = self.get_wts_quant_factor(
            'scale_w',
            np.float32,
            desirable=self.node.type not in self.no_weight_quant_types
            and enable_wts_quant,
        )
        offset_w_array = self.get_wts_quant_factor(
            'offset_w',
            np.int32,
            desirable=self.node.type not in self.no_weight_quant_types
            and enable_wts_quant,
        )
        shift_n_array = self.get_shift_n(
            desirable=(enable_shift_n and self.node.type in self.shift_n_types)
        )
        ...
        if enable_shift_n or enable_wts_quant:
            scale_w_array, offset_w_array, shift_n_array = (
                self.adjust_scale_offset_shiftn(
                    scale_w_array, offset_w_array, shift_n_array
                )
            )
        ...
        # optional cluster center
        if cluster_center_array is not None:
            layer_params[CLUSTER_CENTER] = layer_params.get(CLUSTER_CENTER).astype(
                np.int8
            )
    ...
    def parse_tensor_balance_factor_value(self):
        ...
        tensor_balance_factor_array = self.get_tensor_balance_factor(
            desirable=(self.node.type not in self.no_dmq_balancer_types)
        )
        ...
        if tensor_balance_factor_array.size != tensor_balance_factor_length_expect:
            raise ValueError(
                "tensor_balance_factor's length: {} should be equal to"
                "the channel length of inputs of layer {}: {}."
                "Cannot support to quantize {}".format(
                    tensor_balance_factor_array.size,
                    self.node.name,
                    tensor_balance_factor_length_expect,
                    self.node.name,
                )
            )
        # check tensor_balance_factor value
        if (
            not (tensor_balance_factor_array >= TENSOR_BALANCE_FACTOR_RANGE[0]).all()
            or not (tensor_balance_factor_array <= TENSOR_BALANCE_FACTOR_RANGE[1]).all()
        ):
            raise ValueError(
                "tensor_balance_factor's value shoule be in range[FLT_EPSILON, 1/FLT_EPSILON] "
    ...
    def get_dst_type(self):
        '''get dst type'''
        if hasattr(self.record.value, 'dst_type') and self.record.value.HasField(
            'dst_type'
        ):
            if 'AvgPool' in self.record.key and self.record.value.dst_type == INT4:
                raise RuntimeError(
                    'AvgPool in INT4 not supported. Please Check record file.'
                )
            if self.record.value.dst_type in ('INT8', INT4):
                return self.record.value.dst_type
            raise RuntimeError(
                'Do not support dst_type {} in record file'.format(
    ...
    def get_act_type(self):
        '''get act type'''
        if hasattr(self.record.value, 'act_type') and self.record.value.HasField(
            'act_type'
        ):
            support_act_type = [
                'INT' + str(num_bits) for num_bits in ACT_SUPPORT_NUM_BITS
            ]
            if self.record.value.act_type in support_act_type:
                return self.record.value.act_type
            raise RuntimeError(
                'Do not support act_type {} in record file'.format(
    ...
    def get_wts_type(self):
        '''get wts type'''
        if hasattr(self.record.value, 'wts_type') and self.record.value.HasField(
            'wts_type'
        ):
            if self.record.value.wts_type in (INT4, 'INT6', 'INT7', 'INT8'):
                return self.record.value.wts_type
            raise RuntimeError(
                'Do not support wts_type {} in record file'.format(
    ...
    def get_op_dtype(self, layer_params):
        '''get op dtype'''
        if hasattr(self.record.value, OP_DATA_TYPE) and self.record.value.HasField(
            OP_DATA_TYPE
        ):
            if self.record.value.op_data_type not in ['FLOAT32', 'FLOAT16']:
                raise RuntimeError(
                    f'Do not support op_data_type {self.record.value.op_data_type}'
                )
    ...
    def get_wts_quant_factor(self, name, dtype, desirable=True):
        ...
        if desirable:
            if not getattr(self.record.value, name, None):
                raise RuntimeError(
                    "cannot find {} of layer {} in record_file".format(
                        name, self.get_key()
                    )
                )
            ...
            if getattr(self.record.value, name, None):
                raise RuntimeError(
                    "find undesirable {} of layer {} in record_file".format(
                        name, self.get_key()
                    )
                )
    ...
    def get_tensor_balance_factor(self, desirable=True):
        ...
        if (
            not hasattr(self.record.value, 'tensor_balance_factor')
            or not self.record.value.tensor_balance_factor
        ):
            return None
        if not desirable:
            raise RuntimeError(
    ...
    def get_fakequant_precision_mode(self):
        ...
        ) and self.record.value.HasField('fakequant_precision_mode'):
            if self.record.value.fakequant_precision_mode in [
                'DEFAULT',
                'FORCE_FP16_QUANT',
            ]:
                return self.record.value.fakequant_precision_mode
            raise RuntimeError(
                'Do not support fakequant_precision_mode {} in record file'.format(
                    self.record.value.fakequant_precision_mode
                )
            )
    ...
    def adjust_scale_offset_shiftn(self, scale_w_array, offset_w_array, shift_n_array):
        '''
        ...
        if (
            scale_w_array.size != offset_w_array.size
            or scale_w_array.size != shift_n_array.size
        ):
            raise RuntimeError(
                "{} should be equal in layer {}.".format(err_prefix, operation.name)
            )
        ...
        if scale_w_array.size == 1:
            ...
            if (
                operation.type in RNN_LAYER_TYPE
                and scale_w_array.size == RNN_TENSOR_NUM.get(operation.type)
            ):
                channel_wise = False
            else:
                channel_wise = True
        ...
        shape_expect, scale_length_expect = self.get_possible_length(channel_wise)
        if scale_w_array.size != scale_length_expect:
            raise RuntimeError(
                "{}({}) should be equal to the {} when "
    ...
    def get_possible_length(self, channel_wise=True):
        ...
        if operation.type in ('AvgPool', 'AveragePool'):
            ...
            shape_expect, scale_length_expect = quire_func(self.node, channel_wise)
