...
class AutoChannelPruneSearchBase(ABC):
    def __init__(
        self, graph, input_data, config_item, sesitivity, search_alg, output_config
    ):
        self.graph = graph
        self.input_data = input_data
        self.config_item = config_item
        self.search_records = self.get_search_ops(
            graph, config_item.create_prune_config()
        )
        ...
        for record in search_records:
            ...
            for producer in record.producer:
                if graph_info.get(producer.name):
                    bitops += (
                        graph_info[producer.name][BITOPS]
                        / graph_info[producer.name]['cin']
                    )
            for consumer in record.consumer:
                if graph_info.get(consumer.name):
                    bitops += (
                        graph_info[consumer.name][BITOPS]
                        / graph_info[consumer.name]['cout']
                    )
    ...
    def get_sensitivity(
        self, input_data, test_iteration, search_records, output_nodes=None
    ):
        '''sensitivity write to record'''
        search_graph_info = self.graph_info
        self.sensitivity.setup_initialization(
            (self.graph, search_graph_info), input_data, test_iteration, output_nodes
        )
    ...
    def run(self, input_data, output_nodes=None):
        if not self.search_records:
            LOGGER.logw(
                "No channel prunable structure detected in layers to be searched."
            )
            prune_channel = {}
        else:
            self.get_sensitivity(
                input_data,
                self.config_item.test_iteration,
                self.search_records,
                output_nodes,
            )
            self.cal_channel_bitops(self.graph_info, self.search_records)
            prune_channel = self.search_alg.channel_prune_search(
                self.graph_info,
                self.search_records,
                (
                    self.config_item.compress_ratio,
                    self.config_item.ascend_optimized,
                    self.config_item.max_prune_ratio,
                ),
            )
