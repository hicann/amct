...
class AutoChannelPruneConfigHelper:
        ...
        with open(config_file, 'r') as fid:
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    "it matches with {}!".format(config_file, 'AutoChannelPruneConfig')
                ) from e
    ...
    def create_final_config(self, search_prune_config, output_config):
        ...
        with os.fdopen(
            os.open(output_config, file_flags, file_mode),
            'w',
            encoding='UTF-8',
            newline='',
        ) as fid:
    ...
    def _check_params(self):
        ...
        if self.config_proto.compress_ratio <= 1:
            raise ValueError(
                "compress_ratio not supported. compress_ratio should be larger than 1."
            )
        if self.max_prune_ratio <= 0 or self.max_prune_ratio > 1:
            raise ValueError(
                "max_prune_ratio not supported. max_prune_ratio should be in (0, 1]."
            )
        if self.config_proto.test_iteration < 1:
            raise ValueError(
                "test_iteration not supported. test_iteration should be no smaller than 1."
            )
    ...
    def _check_override_prune_cfg(self):
        ...
        if not os.path.exists(self.override_prune_cfg):
            raise ValueError(
                "The {} in AutoChannelPruneConfig does not exist, please check the file path.".format(
                    self.override_prune_cfg
                )
            )
        ...
        if not set(fields).issubset(set(OVERRIDE_FIELDS)):
            raise ValueError(
                "override_prune_cfg should contain only override or skip config."
            )

        _, retrain_override_layers, prune_override_layers = proto.get_override_layers()
        if retrain_override_layers or not set(
            prune_type for prune_type, layer in prune_override_layers.items() if layer
        ).issubset([FILTER_PRUNE]):
            raise ValueError(
                "override_layer_configs in override_prune_cfg should contain only filter_pruner config."
            )
        if not set(prune_override_layers.get(FILTER_PRUNE)).issubset(
            set(self.support_layers.keys())
        ):
            raise ValueError("some override_layer not in valid_layers for filter prune")

        _, retrain_override_types, prune_override_types = (
            proto.get_override_layer_types()
        )
        if retrain_override_types or not set(
            prune_type for prune_type, types in prune_override_types.items() if types
        ).issubset([FILTER_PRUNE]):
            raise ValueError(
                "override_layer_types in override_prune_cfg should contain only filter_pruner config."
            )
        if not set(prune_override_types.get(FILTER_PRUNE)).issubset(
            set(self.support_types)
        ):
        ...
        for layer, layer_type in self.support_layers.items():
            if (
                layer
                not in prune_override_layers.get(FILTER_PRUNE)
                + regular_prune_skip_layers
                and layer_type
                not in prune_override_types.get(FILTER_PRUNE) + regular_prune_skip_types
            ):
                self.search_layers.append(layer)
        if not self.search_layers:
            raise ValueError(
                "no layer to be searched, please make sure not all layers are overrided or skipped."
            )
    ...
    def _parser_prune_config(self, search_prune_config):
        ...
        """
        # global prune config
        pruner = self.override_proto_config.prune_config.filter_pruner.balanced_l2_norm_filter_prune
        pruner.prune_ratio = 0.3
        pruner.ascend_optimized = self.ascend_optimized
        ...
        for layer_name, prune_config in search_prune_config.items():
            ...
            pruner = (
                proto_override.prune_config.filter_pruner.balanced_l2_norm_filter_prune
            )
            pruner.prune_ratio = prune_ratio
            pruner.ascend_optimized = self.ascend_optimized
