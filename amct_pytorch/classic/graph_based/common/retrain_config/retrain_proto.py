...
class RetrainProtoConfig:
    ...
    def __check_proto_override_types_enable(override_layer_types):
        ...
        for config in override_layer_types:
            if config.HasField('retrain_data_quant_config') or config.HasField(
    ...
    def _parse_n_out_of_m_type(data_type_proto):
        ...
        raise ValueError(
            'n_out_of_m_type is not M4N2, only M4N2 is support in n_out_of_m_type.'
        )
    ...
    def _get_prune_config(config_item):
        ...
        if config_item.HasField('filter_pruner'):
            ...
            if not filter_pruner.HasField('balanced_l2_norm_filter_prune'):
                raise ValueError(
                    'only balanced_l2_norm_filter_prune is support, '
            ...
            if not filter_pruner.balanced_l2_norm_filter_prune.HasField('prune_ratio'):
                raise ValueError(
                    "prune_ratio is required for balanced_l2_norm_filter_prune, please add it."
                )
        ...
        elif hasattr(config_item, 'n_out_of_m_pruner') and config_item.HasField(
            'n_out_of_m_pruner'
        ):
            ...
            if not n_out_of_m_pruner.HasField('l1_selective_prune'):
                raise ValueError(
                    'only l1_selective_prune is support, '
                    ' but l1_selective_prune is not found.'
                )
            algo_config[ALGO] = "l1_selective_prune"
            algo_config['n_out_of_m_type'] = RetrainProtoConfig._parse_n_out_of_m_type(
                n_out_of_m_pruner.l1_selective_prune.n_out_of_m_type
            )
            algo_config['update_freq'] = (
                n_out_of_m_pruner.l1_selective_prune.update_freq
            )
    ...
    def parse_proto_enable(self):
        ...
        if not self.proto_config.HasField(
            'retrain_data_quant_config'
        ) and not self.proto_config.HasField('retrain_weight_quant_config'):
    ...
    def get_skip_layers(self):
        ...
        if repeated_layers:
            LOGGER.logw(
                "Please delete repeated items in skip_layers, "
    ...
    def get_skip_layer_types(self):
        ...
        if repeated_types:
            LOGGER.logw(
                "Please delete repeated items in skip_layer_types, "
    ...
    def get_quant_skip_types(self):
        ...
        if repeated_types:
            LOGGER.logw(
                "Please delete repeated items in quant_skip_types, "
                "repeated items are %s " % (repeated_types),
                module_name="Configuration",
            )
    ...
    def get_override_layers(self):
        ...
        prune_override_layers = {FILTER: [], SELECTIVE: []}
        override_layers = []
    ...
    def get_override_layer_types(self):
        ...
        regular_prune_override_types = {FILTER: [], SELECTIVE: []}
        override_types = []
        for config in self.proto_config.override_layer_types:
            ...
            if self.has_prune and config.HasField('prune_config'):
                if config.prune_config.HasField('filter_pruner'):
                    ...
                    regular_prune_override_types.get(SELECTIVE).append(
                        config.layer_type
                    )
        ...
        if self.capacity.get_value('RETRAIN_MAP_TYPES') is None:
            ...
            return override_types, retrain_override_types, regular_prune_override_types

        # handle ms if RETRAIN_MAP_TYPES exists
        return self.get_trans_override_layer_types(
            override_types, retrain_override_types, regular_prune_override_types
        )

    def get_trans_override_layer_types(
        self, override_types, retrain_override_types, regular_prune_override_types
    ):
        ...
        trans_regular_prune_override_types = {FILTER: [], SELECTIVE: []}
        ...
        for layer_type in regular_prune_override_types.get(FILTER):
            trans_regular_prune_override_types.get(FILTER).extend(
                self._transform_layer_types(layer_type)
            )
        for layer_type in regular_prune_override_types.get(SELECTIVE):
            trans_regular_prune_override_types.get(SELECTIVE).extend(
                self._transform_layer_types(layer_type)
            )
    ...
    def get_regular_prune_skip_layers(self):
        ...
        if repeated_layers:
            LOGGER.logw(
                "Please delete repeated items in regular_prune_skip_layers, "
                "repeated items are %s " % (repeated_layers),
                module_name="Configuration",
            )
    ...
    def get_regular_prune_skip_types(self):
        ...
        if repeated_types:
            LOGGER.logw(
                "Please delete repeated items in regular_prune_skip_types, "
                "repeated items are %s " % (repeated_types),
                module_name="Configuration",
            )
        ...
        if not enable_retrain:
            ...
            for item in retrain_fields:
                if not is_empty(getattr(self.proto_config, item)):
                    raise ValueError(
                        "unexpected {} in config_defination if retrain is disable.".format(
                            item
                        )
                    )
            override_fields = [
            ...
            for config in self.proto_config.override_layer_configs:
                for item in override_fields:
                    if getattr(config, item).ListFields():
                        raise ValueError(
                            "unexpected {} for override_layer_configs in "
            ...
            for config in self.proto_config.override_layer_types:
                for item in override_fields:
                    if getattr(config, item).ListFields():
                        raise ValueError(
                            "unexpected {} for override_layer_types in"
                            " config_defination if retrain is disable.".format(item)
                        )
    ...
    def _check_prune_field(self, enable_prune):
        ...
        if not enable_prune and self.has_prune:
            ...
            for item in retrain_fields:
                if not is_empty(getattr(self.proto_config, item)):
                    raise ValueError(
                        "unexpected {} in config_defination if prune is disable.".format(
                            item
                        )
                    )
    ...
    def _check_retrain_int4_capability(self, data_type, is_weight=False):
        ...
        if not int4_retrain_enable and data_type == INT4:
            raise ValueError("Int4 quantization aware training is not supported.")
    ...
    def _get_retrain_data_config(self, config_item):
        ...
        if retrain_data_config.HasField('clip_max_min'):
            if not retrain_data_config.clip_max_min.HasField(
                'clip_max'
            ) or not retrain_data_config.clip_max_min.HasField('clip_min'):
                raise ValueError("clip_max and clip_min are both required.")
        ...
        if hasattr(retrain_data_config, DST_TYPE):
            if retrain_data_config.HasField(DST_TYPE):
                retrain_data_params[DST_TYPE] = RetrainProtoConfig._parse_data_type(
    ...
    def _get_retrain_weight_config(self, config_item):
        ...
        if config_item.HasField('arq_retrain'):
            ...
            if retrain_weight_config.HasField(CHANNEL_WISE):
                retrain_weight_params[CHANNEL_WISE] = retrain_weight_config.channel_wise
            if hasattr(retrain_weight_config, DST_TYPE):
                if retrain_weight_config.HasField(DST_TYPE):
                    retrain_weight_params[DST_TYPE] = (
                        RetrainProtoConfig._parse_data_type(
            ...
            if retrain_weight_config.HasField(DST_TYPE):
                retrain_weight_params[DST_TYPE] = RetrainProtoConfig._parse_data_type(
                    retrain_weight_config.dst_type
                )
            else:
                retrain_weight_params[DST_TYPE] = INT8
    ...
    def __check_proto_override_enable(self):
        ...
        layer_retrain, layer_prune = self.__check_proto_override_configs_enable(
            override_layer_configs
        )
        layer_types_retrain, layer_types_prune = (
            self.__check_proto_override_types_enable(override_layer_types)
        )
