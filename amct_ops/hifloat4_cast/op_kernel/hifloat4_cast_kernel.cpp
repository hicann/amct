...
public:
    ...
    aifunc void Init(GM_ADDR xmtx_, GM_ADDR out_, int M_, int N_, int MB_) {
        ...
        signbuf = Tensor<float, PUB>(0, BATCH, offset);
        ...
        signmaskbuf1 = Tensor<uint16_t, PUB>(0, 128, offset);
        signmaskbuf2 = Tensor<uint16_t, PUB>(0, 128, offset);
        ...
        nfbuf = Tensor<float, PUB>(0, BATCH, offset);
        nfmaxbuf = Tensor<float, PUB>(0, BATCH, offset);
        zerobuf = Tensor<float, PUB>(0, 64, offset);
        nanbuf = Tensor<float, PUB>(0, 64, offset);
        InitConstants();
    }
    ...
    aifunc void InitConstants() {
        ...
        vector_dup(zerobuf.ptr(), 0.0f, 1, 1, 1, 8, 8);
        vector_dup(((__ubuf__ uint32_t *)nanbuf.ptr()), 0x7FC00000, 1, 1, 1, 8, 8);
        vector_dup(nfbuf.ptr(), 1.0f, BATCH / 64, 1, 1, 8, 8);
        vector_dup(bf16_seven_buf.ptr(), (1.0f / 7.0f), BATCH / 64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        vconv_f322bf16r(bf16_seven_buf_bf16.ptr(), bf16_seven_buf.ptr(), BATCH / 64, 1, 1, 4, 8);
        pipe_barrier(PIPE_V);
    ...
    aifunc void LoadInput() {
        ...
        if (n_ * BATCH + BATCH <= shape.N) {
            copy_gm_to_ubuf(
                xbuf.get(mn_).vptr(), xmtx[m_ * shape.N + n_ * BATCH].vptr(), 0, 1, BATCH * sizeof(T) / 32, 0, 0);
        } else {
        ...
        if constexpr (std::is_same<T, float>::value) {
            ...
            inp_tsr_ = fp32inbuf;
            out_tsr_ = fp32outbuf;
            vconv_bf162f32(inp_tsr_.ptr(), xbuf.get(mn_).ptr(), BATCH / 64, 1, 1, 8, 4);
            pipe_barrier(PIPE_V);
        }
    }
    ...
    aifunc void ZeroTail() {
        ...
        if (n_tail_ > 0) {
            ...
            for (int tail = n_tail_; tail > 0; tail -= 64) {
                if (tail < 64) {
                    uint64_t mask = 1;
                ...
                pipe_barrier(PIPE_V);
                vector_dup(inp_tsr_[BATCH - 64].ptr(), 0.0f, 1, 1, 1, 8, 8);
                pipe_barrier(PIPE_V);
            }
        }
        set_vector_mask((uint64_t)-1, (uint64_t)-1);
    ...
    aifunc void PrepareSignAbs() {
        ...
        pipe_barrier(PIPE_V);
        vabs(absbuf.ptr(), inp_tsr_.ptr(), BATCH / 64, 1, 1, 8, 8);
        vand((__ubuf__ uint16_t *)signbuf.ptr(), (__ubuf__ uint16_t *)inp_tsr_.ptr(), signmaskbuf2.ptr(), BATCH / 64, 1,
            1, 0, 8, 8, 0);
        pipe_barrier(PIPE_V);
        vor((__ubuf__ uint16_t *)signbuf.ptr(), (__ubuf__ uint16_t *)signbuf.ptr(), signmaskbuf1.ptr(), BATCH / 64, 1,
            1, 0, 8, 8, 0);
    }
    ...
    aifunc void HandleNonFinite() {
        ...
        vcmpvs_le((__ubuf__ uint8_t *)exp1maskbuf.ptr(), absbuf.ptr(), 3.4028234663852886e+38f, BATCH / 64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        set_cmpmask(onesbuf.vptr()); // fallback = 1.0 (non-finite side)
        pipe_barrier(PIPE_V);
        vsel(nfbuf.ptr(), zerobuf.ptr(), exp1maskbuf.vptr(), BATCH / 64, 1, 1, 1, 8, 0, 1, 1);
        pipe_barrier(PIPE_V);
        // zero the non-finite magnitudes (re-compare: the first select may consume the mask)
        vcmpvs_le((__ubuf__ uint8_t *)exp2maskbuf.ptr(), absbuf.ptr(), 3.4028234663852886e+38f, BATCH / 64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        set_cmpmask(zerobuf.vptr()); // fallback = 0.0 (non-finite side)
        pipe_barrier(PIPE_V);
        vsel(absbuf.ptr(), absbuf.ptr(), exp2maskbuf.vptr(), BATCH / 64, 1, 1, 1, 8, 8, 1, 1);
        pipe_barrier(PIPE_V);
        // per-64-block poison flag: max of the markers (same reduction as the scale path)
        vcmax(nfmaxbuf.ptr(), nfbuf.ptr(), BATCH / 64, 1, 1, 8, ONLY_VALUE);
        pipe_barrier(PIPE_V);
        vbrcb((__ubuf__ uint32_t *)brcbbuf.ptr(), (__ubuf__ uint32_t *)nfmaxbuf.ptr(), 1, 8, (BATCH + 511) / 512);
        pipe_barrier(PIPE_V);
        vbrcb((__ubuf__ uint32_t *)nfmaxbuf.ptr(), (__ubuf__ uint32_t *)brcbbuf.ptr(), 1, 8, BATCH / 64);
        pipe_barrier(PIPE_V);
    }
    ...
    aifunc void PoisonBlock() {
        // NaN block poison: blocks with any non-finite element get NaN scale, which
        // dequantizes the whole 64-block to NaN (aligned with the HiFloat4 golden).
        // clean (mask=1) -> keep exp0buf; poisoned -> fallback NaN.
        vcmpvs_lt((__ubuf__ uint8_t *)exp1maskbuf.ptr(), nfmaxbuf.ptr(), 0.5f, BATCH / 64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        set_cmpmask(nanbuf.vptr()); // fallback = quiet NaN
        pipe_barrier(PIPE_V);
        vsel(exp0buf.ptr(), exp0buf.ptr(), exp1maskbuf.vptr(), BATCH / 64, 1, 1, 1, 8, 8, 1, 1);
        pipe_barrier(PIPE_V);
    }
    ...
    aifunc void RoundScaleE6M2() {
        ...
        vand((__ubuf__ uint16_t *)sfexpbuf.ptr(), (__ubuf__ uint16_t *)expmask.ptr(),
            (__ubuf__ uint16_t *)exp0buf.ptr(), BATCH / 64, 1, 1, 1, 8, 0, 8);
        pipe_barrier(PIPE_V);
        if constexpr (E6MX == 2) {
    ...
    aifunc void ComputeMantissa() {
        ...
        vdiv(mantbuf.ptr(), absbuf.ptr(), exp2buf.ptr(), BATCH / 64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        ...
        vconv_f322bf16r(bf16_general_buf.ptr(), mantbuf.ptr(), BATCH / 64, 1, 1, 4, 8);
        pipe_barrier(PIPE_V);
        vconv_bf162f32(mantbuf.ptr(), bf16_general_buf.ptr(), BATCH / 64, 1, 1, 8, 4);
        pipe_barrier(PIPE_V);

        vmuls(mantbuf.ptr(), mantbuf.ptr(), (float)(1 << (shape.MB - 1)), BATCH / 64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        vadds(mantbuf.ptr(), mantbuf.ptr(), 0.5f, BATCH / 64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        vconv_f322f32z(mantbuf.ptr(), mantbuf.ptr(), BATCH / 64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
    ...
    aifunc void Reconstruct() {
        ...
        vmul(out_tsr_.ptr(), exp2buf.ptr(), mantbuf.ptr(), BATCH / 64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        vmul(out_tsr_.ptr(), exp1buf.ptr(), out_tsr_.ptr(), BATCH / 64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        vmul(out_tsr_.ptr(), exp0buf.ptr(), out_tsr_.ptr(), BATCH / 64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        vmul(out_tsr_.ptr(), out_tsr_.ptr(), signbuf.ptr(), BATCH / 64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
    }
    ...
    aifunc void StoreOutput() {
        if constexpr (std::is_same<T, float>::value) {
            ...
            vconv_f322bf16a(outbuf.get(mn_).ptr(), out_tsr_.ptr(), BATCH / 64, 1, 1, 4, 8);
            pipe_barrier(PIPE_V);
        }
        ...
        if (n_tail_ > 0) {
            ...
            copy_ubuf_to_gm(
                out[m_ * shape.N + n_ * BATCH].vptr(), outbuf.get(mn_).vptr(), 0, 1, BATCH * sizeof(T) / 32, 0, 0);
        }
        output_empty.set();
    }
