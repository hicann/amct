...
if __name__ == '__main__':
    ...
    samples = get_calib_dataset(tokenizer=enc, n_samples=512, block_size=model.seqlen)
    samples = torch.cat(samples, dim=0)[:1, :]

    # Phase1: quantize model using Quantile algorithm
    cfg = amct.HIFP8_QUANTILE_CFG
    ...
    # Phase3: skip convert in environments that do not support hifloat.
    # Use the quantized model from amct.quantize for simulation testing.
    torch_npu.npu.empty_cache()

    # Phase4: Test ppl result with quantized model
